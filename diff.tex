...
%DIF DEL main_conditional_accept_102022.tex   Wed Apr 20 10:43:41 2022
%DIF ADD main.tex                             Tue Oct 11 14:23:55 2022
...
At every \textit{instance} $t$ (used to distinguish from an estimation \textit{timestep}), the navigator first broadcasts $m$ weights $\omega_j^{(t)}, j\in\{1,\dots,m\}$ to all sensors $i\in\{1,\dots,n\}$, who individually compute linear combinations \DIFdelbegin \DIFdel{$l^{(t)}_i=\sum^m_{j=1}a_{j,i}^{(t)}\omega_i^{(t)}$ }\DIFdelend \DIFaddbegin \DIFadd{$l^{(t)}_i=\sum^m_{j=1}a_{i,j}^{(t)}\omega_j^{(t)}$ }\DIFaddend based on their measurement data $a_{j,i}$. Linear combinations are then sent back to the navigator, who computes their sum $\sum^n_{i=1}l^{(t)}_{i}$. This two-step linear combination aggregation protocol has been visually displayed in figure \ref{fig:agg_steps}.
\begin{figure}[htbp]
\centering
...
\begin{LaTeXdescription}
    ...
    \DIFdelbegin %DIFDELCMD < \item[Honest-but-Curious Sensors] %%%
\DIFdelend \DIFaddbegin \item[\DIFadd{Honest-but-Curious Attackers}] \DIFaddend We adopt the honest-but-curious attacker model\DIFdelbegin \DIFdel{for all involved sensors , meaning that they }\DIFdelend \DIFaddbegin \DIFadd{. The navigator and sensors are corruptable but }\DIFaddend follow the localisation procedure correctly\DIFdelbegin \DIFdel{but may store or }\DIFdelend \DIFaddbegin \DIFadd{, while attackers may }\DIFaddend use any gained sensitive information.
...
\begin{LaTeXdescription}
    ...
    \DIFdelbegin %DIFDELCMD < \item[$\mathsf{CombEnc}(t, pk_0, sk_i, \mathcal{E}(\omega_1^{(t)}),\dots,\mathcal{E}(\omega_m^{(t)}), a^{(t)}_{i,1},\dots,a^{(t)}_{i,m})$] %%%
\DIFdelend \DIFaddbegin \item[\DIFadd{$\mathsf{CombEnc}(t, pk_0, sk_i, \mathcal{E}_{pk_0}(\omega_1^{(t)}),\!...,\mathcal{E}_{pk_0}(\omega_m^{(t)}),a^{(t)}_{i,1},\!...,a^{(t)}_{i,m})\hspace*{1.2pt}$}] \DIFaddend At instance $t$, sensor $i$ computes and obtains the encrypted linear combination \DIFdelbegin \DIFdel{denoted $l^{(t)}_i = \mathcal{E}_{pk_0,sk_i}(\sum^m_{j=1}a^{(t)}_{i,j}\omega^{(t)}_j)$ using its }\DIFdelend \DIFaddbegin \DIFadd{$\mathcal{E}_{pk_0,sk_i}(\sum^m_{j=1}a^{(t)}_{i,j}\omega^{(t)}_j)$ with }\DIFaddend secret key $sk_i$.
    \DIFdelbegin %DIFDELCMD < \item[$\mathsf{AggDec}(t, pk_0, sk_0, l^{(t)}_1,\dots,l^{(t)}_n)$] %%%
\DIFdelend \DIFaddbegin \item[\DIFadd{$\mathsf{AggDec}(t, pk_0, sk_0, \mathcal{E}_{pk_0,sk_1}(l^{(t)}_1),\dots,\mathcal{E}_{pk_0,sk_n}(l^{(t)}_n))$}] \DIFaddend At instance $t$, the navigator computes the \DIFaddbegin \DIFadd{plaintext }\DIFaddend aggregation of linear combinations \DIFdelbegin \DIFdel{$\sum^{n}_{i=1}l_i^{(t)}=\sum^{n}_{i=1}\sum^{m}_{j=1} a^{(t)}_{i,j}\omega^{(t)}_j$ }\DIFdelend \DIFaddbegin \DIFadd{$\sum^{n}_{i=1}l_i^{(t)}$ }\DIFaddend using its public and private keys $pk_0$, $sk_0$.
\end{LaTeXdescription}
