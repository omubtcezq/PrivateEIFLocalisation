...
%DIF DEL main_conditional_accept_022022.tex   Tue Feb  1 16:43:23 2022
%DIF ADD main.tex                             Wed Apr 20 10:43:41 2022
...
Typical cryptographic secrecy involves hiding all transferred data such that external parties in the communication network learn no new information from acquired encryptions. This can be achieved with common symmetric and public-key encryption \DIFdelbegin \DIFdel{scheme such as AES \mbox{%DIFAUXCMD
\cite{gueronIntelAdvancedEncryption2010} }\hskip0pt%DIFAUXCMD
and RSA }\DIFdelend \DIFaddbegin \DIFadd{schemes such as the Advanced Encryption Standard (AES) \mbox{%DIFAUXCMD
\cite{gueronIntelAdvancedEncryption2010} }\hskip0pt%DIFAUXCMD
and the Rivest-Shamir-Adleman cryptosystem (RSA) }\DIFaddend \cite{rivestMethodObtainingDigital1978}, respectively. \DIFaddbegin \DIFadd{These scenarios, however, imply trust between the encrypting and decrypting parties in the network. }\DIFaddend In some cases\DIFdelbegin \DIFdel{however, partial data leakage or encrypted data processing is required for achieving a desiredgoal which }\DIFdelend \DIFaddbegin \DIFadd{, this assumption cannot be made and control over what can be learnt or performed with encryptions is desired. This }\DIFaddend has led to several homomorphic and functional encryption schemes \cite{paillierPublicKeyCryptosystemsBased1999,shiPrivacyPreservingAggregationTimeSeries2011,joyeScalableSchemePrivacyPreserving2013,chotardDecentralizedMultiClientFunctional2018} finding uses in \DIFdelbegin \DIFdel{signal processing or }\DIFdelend \DIFaddbegin \DIFadd{various signal processing and }\DIFaddend localisation tasks. In \cite{alanwarPrOLocResilientLocalization2017}, homomorphic encryption is used to make time-independent model-free location estimates where an estimator \DIFdelbegin \DIFdel{does not }\DIFdelend \DIFaddbegin \DIFadd{cannot }\DIFaddend learn sensor measurements or locations. In \cite{aristovEncryptedMultisensorInformation2018}, similar secrecy is achieved with a linear Kalman filter when a hierarchical sensor network is present. In \cite{shiPrivacyPreservingAggregationTimeSeries2011,joyeScalableSchemePrivacyPreserving2013}, privacy-preserving aggregation schemes are presented as a means to compute total \DIFdelbegin \DIFdel{powergrid }\DIFdelend \DIFaddbegin \DIFadd{power grid }\DIFaddend usage without disclosing individual contributions, while in \cite{alexandruEncryptedCooperativeControl2019,alexandruPrivateWeightedSum2020}, \DIFdelbegin \DIFdel{centralised and hidden weighted sum aggregations, }\DIFdelend \DIFaddbegin \DIFadd{private Weighted Sum Aggregation with centralised or hidden weights (}\DIFaddend pWSAc and pWSAh, \DIFaddbegin \DIFadd{respectively) }\DIFaddend are introduced as a means for computing \DIFdelbegin \DIFdel{local }\DIFdelend control inputs in a distributed \DIFdelbegin \DIFdel{environment }\DIFdelend \DIFaddbegin \DIFadd{network }\DIFaddend without learning individual \DIFdelbegin \DIFdel{inputs}\DIFdelend \DIFaddbegin \DIFadd{contributions}\DIFaddend .

Our contribution in this work presents a range-only localisation method meeting formal cryptographic requirements that ensure sensors keep their measurements\DIFaddbegin \DIFadd{, sensor variances }\DIFaddend and locations private while the navigator keeps its estimates private. We first define a novel \DIFdelbegin \DIFdel{notion for private }\DIFdelend \DIFaddbegin \DIFadd{cryptographic notion for }\DIFaddend linear combination aggregation and present an implementation \DIFdelbegin \DIFdel{that satisfies these }\DIFdelend \DIFaddbegin \DIFadd{satisfying the }\DIFaddend requirements, before using it to derive a filter based on the Extended Kalman Filter with no hierarchical sensor layout assumptions. The linear combination aggregation scheme \DIFdelbegin \DIFdel{we put forward }\DIFdelend is in principle similar to the pWSAh scheme in \cite{alexandruPrivateWeightedSum2020}, however, a formal definition with different communication assumptions and leakages is given, crucial for its cryptographic security. To the best of the authors' knowledge, no existing method for Bayesian state estimation using range-only sensors and meeting the desired privacy requirements exists.

We motivate this scenario with \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{an }\DIFaddend example of vehicle localisation in the presence of privately owned measurement stations. While the intention of \DIFdelbegin \DIFdel{measurement }\DIFdelend stations is the accurate navigation of passing vehicles, it may be reasonable to desire identifying \DIFdelbegin \DIFdel{location or hardware details}\DIFdelend \DIFaddbegin \DIFadd{details, such as hardware specifications or physical locations, }\DIFaddend to remain unknown to the other \DIFdelbegin \DIFdel{present }\DIFdelend stations and the navigator. Similarly, a navigator may not wish to disclose their \DIFdelbegin \DIFdel{most }\DIFdelend accurate location estimates to untrusted third-party measurers.

In section \ref{sec:problem_statement}, we introduce both the cryptographic and estimation problems considered in this work, before giving some relevant preliminaries in section \ref{sec:preliminaries}. Section \ref{sec:lcao_scheme} proposes a cryptographic scheme meeting our desired security properties\DIFaddbegin \DIFadd{, }\DIFaddend and section \ref{sec:priv_localisation} gives a solution to the estimation problem making use of this scheme. Simulation results are discussed in section \ref{sec:sim_and_results}\DIFaddbegin \DIFadd{, }\DIFaddend and concluding remarks are made in section \ref{sec:conclusion}.
...
\subsection{Notation}
Throughout this work, we make use of the following notation. Underlined characters, $\vec{a}$, denote vectors, uppercase bold characters, $\mat{A}$, denote matrices. $\mat{A}^{-1}$ is the matrix inverse while $\mat{A}^\top$ is its transpose. The expected value of a random variable is denoted \DIFaddbegin \DIFadd{by }\DIFaddend $\mathsf{E}[\cdot]$, while the variance of a random scalar and covariance of a random vector by $\mathsf{Var}[\cdot]$ and $\mathsf{Cov}[\cdot]$, respectively. $|a|$ denotes absolute value, $\lVert\vec{a}\rVert$ the vector norm, $\{\dots\}$ is used for sets and $\langle\dots\rangle$ for ordered sequences. When estimating a state, notation \DIFdelbegin \DIFdel{$\hat{\vec{x}}_{k|l}$ }\DIFdelend \DIFaddbegin \DIFadd{$\hat{\vec{x}}_{k|e}$ }\DIFaddend will denote an estimate of $\vec{x}$ at timestep $k$ given measurements from timesteps up to and including \DIFdelbegin \DIFdel{$l$}\DIFdelend \DIFaddbegin \DIFadd{timestep $e$}\DIFaddend . When discussing cryptography, \DIFdelbegin \DIFdel{$\mathcal{E}_{k}(\cdot)$ and $\mathcal{D}_{k}(\cdot)$ }\DIFdelend \DIFaddbegin \DIFadd{$\mathcal{E}_{sk}(\cdot)$ and $\mathcal{D}_{sk}(\cdot)$ }\DIFaddend will denote encryption and decryption with key \DIFdelbegin \DIFdel{$k$}\DIFdelend \DIFaddbegin \DIFadd{$sk$}\DIFaddend , respectively, with \DIFdelbegin \DIFdel{$k$ }\DIFdelend \DIFaddbegin \DIFadd{$sk$ }\DIFaddend ommited when inferrable from context. \DIFdelbegin \DIFdel{$\mathbb{Z}_n$ }\DIFdelend \DIFaddbegin \DIFadd{$\mathbb{Z}_N$ }\DIFaddend is used for the set of integers modulo \DIFdelbegin \DIFdel{$n$ and $\mathbb{Z}^*_n$ }\DIFdelend \DIFaddbegin \DIFadd{$N$ and $\mathbb{Z}^*_N$ }\DIFaddend for its multiplicative group. $\mathsf{lcm}(\cdot,\cdot)$ is the lowest common denominator, $\|$ the binary concatination operator\DIFaddbegin \DIFadd{, }\DIFaddend and the term \textit{negligible function} refers to its cryptographic definition in \cite{katzIntroductionModernCryptography2008}.
...
In this work, we consider the context of privacy-preserving range sensor navigation, where we want no sensor to learn \DIFaddbegin \DIFadd{any }\DIFaddend information about the navigator or other sensors beyond their local measurements, and the navigator \DIFdelbegin \DIFdel{to learn no }\DIFdelend \DIFaddbegin \DIFadd{not to learn any }\DIFaddend information about individual sensors beyond its location estimate. The problem is two-fold, in that we require explicit cryptographic requirements with a suitable encryption scheme meeting them \DIFdelbegin \DIFdel{, }\DIFdelend as well as an estimation scheme that can use the \DIFdelbegin \DIFdel{scheme }\DIFdelend \DIFaddbegin \DIFadd{encryption }\DIFaddend in the context of range-only navigation.

To give a formal cryptographic requirement in a distributed setting, we must first consider the communication requirements of our context \DIFdelbegin \DIFdel{, }\DIFdelend and define the attacker capabilities and the desired security of a suitable encryption scheme. In this section, we will define a communication protocol and the relevant formal definition of security we aim to achieve, followed by the estimation problem to which we will apply it.
...
\end{LaTeXdescription}
While indistinguishable weights can be achieved by encrypting weights with an encryption scheme meeting the notion of Indistinguishability under the Chosen Plaintext Attack (IND-CPA) \cite{katzIntroductionModernCryptography2008}, the novel notion of Linear Combination Aggregator Obliviousness (LCAO) has been formalised as a typical cryptographic game between attacker and challenger in appendix\DIFaddbegin \DIFadd{~}\DIFaddend \ref{app:lcao}. Lastly, we conclude the cryptographic problem definition with the following important remark.
\begin{remark}
    A leakage function including weights from the navigator requires extra care to be taken when giving its definition. If an attacker compromises the navigator, they have control over the weights, and therefore the leakage function. We note that in the leakage function above, $\sum^n_{i=1}\sum^m_{j=1}a^{(t)}_{i,j}\omega^{(t)}_j$, an individual sum weighted by the same weight may be \DIFdelbegin \DIFdel{learned }\DIFdelend \DIFaddbegin \DIFadd{learnt }\DIFaddend by an attacker, \textit{e.g.}, $\sum^n_{i=1}a^{(t)}_{i,1}$ given weights $(1,0,\dots,0)$, but that individual sensor values $a^{(t)}_{i,j}$ remain private due to the assumption of a consistent broadcast.
\end{remark}
...
\end{equation}
where $\lambda = \mathsf{lcm}(p-1, q-1)$ and \DIFdelbegin \DIFdel{$L(u) = \frac{u-1}{N}$}\DIFdelend \DIFaddbegin \DIFadd{$L(\psi) = \frac{\psi-1}{N}$}\DIFaddend .
...
\begin{equation}
    sk_0 = -\sum^{n}_{i=1}sk_i\DIFdelbegin %DIFDELCMD < \pmod{N^2}%%%
\DIFdelend \,,
\end{equation}
...
%DIFDELCMD <         &H(t)^{sk_0}\prod^{n}_{i=1}c_{i,t} \pmod{N^2} \\
%DIFDELCMD <         \equiv &H(t)^{sk_0}\prod^{n}_{i=1}(N+1)^{a_{i,t}} H(t)^{sk_i} \pmod{N^2} \\
%DIFDELCMD <         \equiv &H(t)^{\sum^n_{j=0}sk_j} \prod^{n}_{i=1}(N+1)^{a_{i,t}} \pmod{N^2} \\
%DIFDELCMD <         \equiv &(N+1)^{\sum^n_{i=1}a_{i,t}} \pmod{N^2}\,,
%DIFDELCMD <     \end{split}%%%
...
\begin{equation}
    \mathbb{Q}_{M,\phi} = \left\{\DIFdelbegin \DIFdel{q }\DIFdelend \DIFaddbegin \DIFadd{o }\DIFaddend \,\middle|\, \phi \DIFdelbegin \DIFdel{q }\DIFdelend \DIFaddbegin \DIFadd{o }\DIFaddend \in \mathbb{N} \wedge -\left\lfloor\frac{M}{2}\right\rfloor \leq \phi \DIFdelbegin \DIFdel{q }\DIFdelend \DIFaddbegin \DIFadd{o }\DIFaddend < \left\lfloor\frac{M}{2}\right\rfloor \right\}\,,
\end{equation}
and we can quantize any real number $a$ by taking the nearest rational \DIFdelbegin \DIFdel{$q \in \mathbb{Q}_{M,\phi}$}\DIFdelend \DIFaddbegin \DIFadd{$o \in \mathbb{Q}_{M,\phi}$}\DIFaddend , that is, \DIFdelbegin \DIFdel{$\argmin_{q\in\mathbb{Q}_{M,\phi}} |a-q|$}\DIFdelend \DIFaddbegin \DIFadd{$\argmin_{o\in\mathbb{Q}_{M,\phi}} |a-o|$}\DIFaddend . In this form, mapping rationals $\mathbb{Q}_{M,\phi}$ to an encryption range $\mathbb{Z}_N$ is achieved by choosing $M=N$ and handling negatives by modulo arithmetic. Additionally, we note that the Q number format requires a precision factor $\phi$ to be removed after each encoded multiplication. This is captured by a third parameter $d$; the number of \DIFaddbegin \DIFadd{additional }\DIFaddend precision factors present in encodings.
...
\end{equation}
In practice, $N$ is typically very large ($N>2^{1024}$) and this condition can be ignored when $M=N$\DIFaddbegin \DIFadd{, as $\phi$ can be made sufficiently large to make quantisation errors negligible}\DIFaddend .
...
\begin{LaTeXdescription}
    \item[$\mathsf{Setup}(\kappa)$] On input parameter $\kappa$, generate two equal-length, sufficiently large, primes $p$ and $q$, and compute $N=pq$. Define a hash function $H:\mathbb{Z} \rightarrow \mathbb{Z}_{N^2}^*$, choose the number of weights to combine, $m>1$, and set public parameter $\mathsf{pub}=H$, navigator public key $pk_0 = N$ and navigator private key $sk_0=(p,q)$. Sensor secret keys are generated by choosing $sk_i,\,i\in\{1,\dots,n-1\}$ uniformly from $\mathbb{Z}_{N^2}$ and setting the last key \DIFdelbegin \DIFdel{as $sk_n = -\sum^{n-1}_{i=1}sk_i \pmod{N^2}$}\DIFdelend \DIFaddbegin \DIFadd{to $sk_n = -\sum^{n-1}_{i=1}sk_i$}\DIFaddend .
    ...
    \DIFdelbegin %DIFDELCMD < \item[$\mathsf{CombEnc}(t_{k,\tau}, pk_0, sk_i, \mathcal{E}(\omega_1^{(k,\tau)})\dots , a^{(k,\tau)}_{i,1}\dots )$] %%%
\DIFdel{At }\DIFdelend \DIFaddbegin \item[\DIFadd{$\mathsf{CombEnc}(t_{k,\tau}, pk_0, sk_i, \mathcal{E}_{pk_0}(\omega_1^{(k,\tau)})\dots , a^{(k,\tau)}_{i,1}\dots )$}] \DIFadd{At the }\DIFaddend instance $t_{k,\tau}$, encrypted linear combination is \DIFdelbegin \DIFdel{computed as 
    ...
    \begin{equation*}
        \DIFdelbegin %DIFDELCMD < \begin{split}
%DIFDELCMD <             l^{(k,\tau)}_i &= H(t_{k,\tau})^{sk_i}\prod^{m}_{j=1}\mathcal{E}_{pk_0}(\omega^{(k,\tau)}_j)^{a^{(k,\tau)}_{i,j}} \pmod{N^2} \\
%DIFDELCMD <             &= H(t_{k,\tau})^{sk_i}\prod^{m}_{j=1}\mathcal{E}_{pk_0}(a^{(k,\tau)}_{i,j}\omega^{(k,\tau)}_j) \pmod{N^2} \\
%DIFDELCMD <             &= H(t_{k,\tau})^{sk_i}\cdot \\
%DIFDELCMD <             &\qquad \prod^{m}_{j=1}(N+1)^{a^{(k,\tau)}_{i,j}\omega^{(k,\tau)}_j} \rho^{N}_{j} \pmod{N^2} \\
%DIFDELCMD <             &= H(t_{k,\tau})^{sk_i}\cdot \\
%DIFDELCMD <             &\qquad (N+1)^{\sum^{m}_{j=1}a^{(k,\tau)}_{i,j}\omega^{(k,\tau)}_j} \rho_{i}^{N} \pmod{N^2}\,,
%DIFDELCMD <         \end{split}%%%
\DIFdelend \DIFaddbegin \begin{split}
        ...
        \end{split}\DIFaddend 
    \end{equation*}
    for some values \DIFdelbegin \DIFdel{$\rho_i,\rho_j \in \mathbb{Z}_N,\,j\in\{1,\dots,m\}$}\DIFdelend \DIFaddbegin \DIFadd{$\rho_j \in \mathbb{Z}_N,\,j\in\{1,\dots,m\}$ and $\tilde{\rho}_i=\prod^{m}_{j=1}\rho_j$}\DIFaddend . Here, \DIFdelbegin \DIFdel{$\rho_i^N$ }\DIFdelend \DIFaddbegin \DIFadd{$\tilde{\rho}_i^N$ }\DIFaddend and $H(t_{k,\tau})^{sk_i}$ can be considered the noise terms corresponding to the two levels of encryption from $pk_0$ and $sk_i$, respectively.
    ...
    \end{equation}
    with $\lambda = \mathsf{lcm}(p-1, q-1)$ and \DIFdelbegin \DIFdel{$L(u) = \frac{u-1}{N}$}\DIFdelend \DIFaddbegin \DIFadd{$L(\psi) = \frac{\psi-1}{N}$}\DIFaddend . The correctness of the aggregation can be seen from
    \begin{align*}
        \DIFdelbegin %DIFDELCMD < \begin{split}
%DIFDELCMD <         l^{(k,\tau)} &= \prod^n_{i=1}H(t_{k,\tau})^{sk_i}\cdot \\
%DIFDELCMD <         &\qquad (N+1)^{\sum^{m}_{j=1}a^{(k,\tau)}_{i,j}\omega^{(k,\tau)}_j}\rho_i^N \!\!\pmod{N^2}
%DIFDELCMD <         \end{split}%%%
\DIFdelend \DIFaddbegin \begin{split}
        ...
        \end{split}\DIFaddend \\
        \DIFdelbegin %DIFDELCMD < \begin{split}
%DIFDELCMD <             &= H(t_{k,\tau})^{\sum^n_{i=1}sk_i}\cdot \\
%DIFDELCMD <             &\qquad \prod^n_{i=1}(N+1)^{\sum^{m}_{j=1}a^{(k,\tau)}_{i,j}\omega^{(k,\tau)}_j}\rho_i^N \!\!\pmod{N^2}
%DIFDELCMD <         \end{split}%%%
\DIFdelend \DIFaddbegin \begin{split}
            &= H(t_{k,\tau})^{\sum^n_{i=1}sk_i}\cdot \\
            &\qquad \prod^n_{i=1}(N+1)^{\sum^{m}_{j=1}a^{(k,\tau)}_{i,j}\omega^{(k,\tau)}_j}\tilde{\rho}_i^N \!\!\pmod{N^2}
        \end{split}\DIFaddend \\
        &= (N+1)^{\sum^n_{i=1}\sum^{m}_{j=1}a^{(k,\tau)}_{i,j}\omega^{(k,\tau)}_j}\DIFdelbegin \DIFdel{\rho'}\DIFdelend \DIFaddbegin \tilde{\rho}\DIFaddend ^N \!\!\pmod{N^2}\,,
    \end{align*}
    for some values \DIFdelbegin \DIFdel{$\rho_i,\rho' \in \mathbb{Z}_N,\,i\in\{1,\dots,n\}$}\DIFdelend \DIFaddbegin \DIFadd{$\tilde{\rho}_i \in \mathbb{Z}_N,\,i\in\{1,\dots,n\}$ and $\tilde{\rho}=\prod^{n}_{i=1}\tilde{\rho}_i$}\DIFaddend .
\end{LaTeXdescription}
...
\begin{equation}
    \DIFdelbegin %DIFDELCMD < \begin{split}
%DIFDELCMD <         &\hat{\vec{y}}_{k|k} = \hat{\vec{y}}_{k|k-1} + \\
%DIFDELCMD <         &\quad \sum^n_{i=1}\mat{H}^\top_{k,i} r^{-1}_i (z_{k,i} - h_i(\hat{\vec{x}}_{k|k-1}) + \mat{H}_{k,i}\hat{\vec{x}}_{k|k-1})
%DIFDELCMD <     \end{split}%%%
\DIFdelend \DIFaddbegin \begin{split}
    ...
    \end{split}\DIFaddend  \label{eqn:eif_info_vec_update}
\end{equation}
and
\begin{equation}
...
\begin{equation}
    \vec{i}_{k,i} = \mat{H}^\top_{k,i} r^{-1}_i \DIFdelbegin \DIFdel{(}\DIFdelend \DIFaddbegin \left(\DIFaddend z_{k,i} - h_i(\hat{\vec{x}}_{k|k-1}) + \mat{H}_{k,i}\hat{\vec{x}}_{k|k-1}\DIFdelbegin \DIFdel{) }\DIFdelend \DIFaddbegin \right) \DIFaddend \label{eqn:measurement_vec}
\end{equation}
and the measurement matrix
\begin{equation}
    \mat{I}_{k,i} = \mat{H}^\top_{k,i} r^{-1}_i \mat{H}_{k,i}\,, \label{eqn:measurement_mat}
\end{equation}
namely, their measurements $z_{k,i}$, measurement variances $r_{k,i}$ and locations $\vec{s}_i$; captured in measurement functions $h_i$ and Jacobians $\mat{H}_{k,i}$. \DIFdelbegin \DIFdel{However, }\DIFdelend \DIFaddbegin \DIFadd{To compute }\DIFaddend $\vec{i}_{k,i}$ and $\mat{I}_{k,i}$\DIFdelbegin \DIFdel{also require }\DIFdelend \DIFaddbegin \DIFadd{, however, }\DIFaddend the current predicted state estimate $\hat{\vec{x}}_{k|k-1}$ \DIFdelbegin \DIFdel{to be computed }\DIFdelend \DIFaddbegin \DIFadd{is also required }\DIFaddend (in $h_i$ and $\mat{H}_{k,i}$). \DIFdelbegin \DIFdel{For this reason}\DIFdelend \DIFaddbegin \DIFadd{Therefore}\DIFaddend , our goal is to \DIFdelbegin \DIFdel{reformulate }\DIFdelend \DIFaddbegin \DIFadd{rearrange }\DIFaddend \eqref{eqn:measurement_vec} and \eqref{eqn:measurement_mat} \DIFdelbegin \DIFdel{to be computable at each sensor $i$ }\DIFdelend as a linear combination of functions of \DIFdelbegin \DIFdel{the navigator state estimate }\DIFdelend \DIFaddbegin \DIFadd{$\hat{\vec{x}}_{k|k-1}$ }\DIFaddend (the navigator weights) \DIFaddbegin \DIFadd{computable at each sensor $i$}\DIFaddend , to be subsequently aggregated at the navigator. Application of the linear combination aggregation scheme proposed \DIFdelbegin \DIFdel{would in turn guarantee }\DIFdelend \DIFaddbegin \DIFadd{in turn guarantees }\DIFaddend that sensors do not learn the navigator state, and the navigator learns only the aggregation required for updating its state estimate in \eqref{eqn:eif_info_vec_update} and \eqref{eqn:eif_info_mat_update}.
...
The first thing we notice when wanting to \DIFdelbegin \DIFdel{decompose }\DIFdelend \DIFaddbegin \DIFadd{rearrange }\DIFaddend $\vec{i}_{k,i}$ and $\mat{I}_{k,i}$ to a linear combination of functions of $\hat{\vec{x}}_{k|k-1}$, is that $h_i$ cannot be rearranged in this way due to the present square-root. Similarly, the Jacobian of $h_i$ at $\hat{\vec{x}}_{k|k-1}$,
\begin{equation}
    \mat{H}_{k,i} = 
    \begin{bmatrix}
        ...
        0 \\
        \vdots
    \end{bmatrix}\DIFaddbegin \DIFadd{^\top}\DIFaddend \,,
\end{equation}
...
\begin{equation}
    ...
    \begin{bmatrix}
        ...
        0 \\
        \vdots
    \end{bmatrix}\DIFaddbegin \DIFadd{^\top}\DIFaddend \,. \label{eqn:modified_jacobian}
\end{equation}
...
\begin{align*}
    \DIFdelbegin %DIFDELCMD < \begin{split}
%DIFDELCMD <         \alpha_i^{(k,1)} &= (2r_{k,i}^{\prime-1})\hat{x}_{k|k-1}^3 + (2r_{k,i}^{\prime-1})\hat{x}_{k|k-1}\hat{y}_{k|k-1}^2 \\
%DIFDELCMD <         &\quad+ (-r_{k,i}^{\prime-1}s_{x,i})\hat{x}_{k|k-1}^2 + (-2r_{k,i}^{\prime-1}s_{x,i})\hat{y}_{k|k-1}^2 \\
%DIFDELCMD <         &\quad+ (2r_{k,i}^{\prime-1}z'_{k,i})\hat{x}_{k|k-1} + (-2r_{k,i}^{\prime-1}s_{x,i}^2)\hat{x}_{k|k-1}\\
%DIFDELCMD <         &\quad+ (-2r_{k,i}^{\prime-1}s_{y,i}^2)\hat{x}_{k|k-1} + (2r_{k,i}^{\prime-1}s_{x,i}^3) \\
%DIFDELCMD <         &\quad+ (2r_{k,i}^{\prime-1}s_{x,i}s_{y,i}^2) + (-2r_{k,i}^{\prime-1}s_{x,i} z'_{k,i})\ \textrm{and}
%DIFDELCMD <     \end{split}%%%
\DIFdelend \DIFaddbegin \begin{split}
...
\begin{align*}
    ...
    \end{split}
\end{align*}
The above rearrangements give $\vec{i}'_{k,i}$ and $\mat{I}'_{k,i}$ as linear \DIFdelbegin \DIFdel{comabinations }\DIFdelend \DIFaddbegin \DIFadd{combinations }\DIFaddend of elements in
\begin{equation}
    \begin{split}
...
\begin{algorithmic}[1]
    ...
    \ \DIFdelbegin \DIFdel{$\mathcal{E}_{pk_0}(\mathsf{E}_{0}(\hat{x}^2_{k|k-1}))^{\mathsf{E}_{0}(-r_{k, i}^{\prime-1}s_{x,i})}\cdot$}\DIFdelend \DIFaddbegin \DIFadd{$\mathcal{E}_{pk_0}(\mathsf{E}_{0}(\hat{x}^2_{k|k-1}))^{\mathsf{E}_{0}(-2r_{k, i}^{\prime-1}s_{x,i})}\cdot$}\DIFaddend \par
...
\end{figure}
We can see \DIFdelbegin \DIFdel{from the figure }\DIFdelend that the predominant computational costs stem from cryptographic computations and are directly dependent on the chosen key size. \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{In practice, choosing a }\DIFaddend key size should \DIFdelbegin \DIFdel{be chosen such that sufficient security is achieved, and the current recommendation, when }\DIFdelend \DIFaddbegin \DIFadd{take into account the duration of secrecy and the secret key lifetime. When }\DIFaddend relying on the DCRA for security\DIFdelbegin \DIFdel{(difficulty of factorising $N$), }\DIFdelend \DIFaddbegin \DIFadd{, the current recommendation for encrypting government documents }\DIFaddend is the use of $2048$ bit length keys \cite{barkerRecommendationPairwiseKey2019}. For our implementation \DIFdelbegin \DIFdel{of the filter, run on the }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend aforementioned hardware, this results in a filter update \DIFdelbegin \DIFdel{computation duration of roughly }\DIFdelend \DIFaddbegin \DIFadd{roughly every }\DIFaddend $1.7s$. \DIFaddbegin \DIFadd{In a scenario where sensors are mobile and past navigations can be made public, reduced key sizes can be considered, while a further decrease in computation time could be achieved with code optimisations and more powerful hardware.
}\DIFaddend 
...
\begin{proof}
    ...
    \begin{LaTeXdescription}
        ...
        \begin{LaTeXdescription}
            ...
            \end{equation*}
            to $\mathcal{A}$.
            \item[\textit{Combine Query}] When $\mathcal{A}$ submits combine query $(i, t, a^{(t)}_{i,1},\dots,a^{(t)}_{i,m})$, choose weights $\omega^{(t)}_j,j\in\{1,\dots,m\}$ if not already chosen for the instance $t$, and make an AO encryption query $(i, t, \sum^m_{j=1}a^{(t)}_{i,j}\omega^{(t)}_j)$ to $\mathcal{C}$. The received response will be of the form $(N+1)^{\sum^m_{j=1}a^{(t)}_{i,j}\omega^{(t)}_j}H(t)^{sk_i}$; multiply it by \DIFdelbegin \DIFdel{$\rho^N$ }\DIFdelend \DIFaddbegin \DIFadd{$\tilde{\rho}^N$ }\DIFaddend for a random \DIFdelbegin \DIFdel{$\rho \in \mathbb{Z}_N$ and return 
            }\DIFdelend \DIFaddbegin \DIFadd{$\tilde{\rho} \in \mathbb{Z}_N$ and return 
            }\DIFaddend \begin{equation*}
                (N+1)^{\sum^m_{j=1}a^{(t)}_{i,j}\omega^{(t)}_j}\DIFdelbegin \DIFdel{\rho}\DIFdelend \DIFaddbegin \tilde{\rho}\DIFaddend ^N H(t)^{sk_i} \pmod{N^2}
            \end{equation*}
            to $\mathcal{A}$.
        ...
        \end{equation*}
        for an unknown $b \in \{0,1\}$. Multiply series elements by \DIFdelbegin \DIFdel{$\rho_i^N,\,i\in\{1,\dots,n\}$ }\DIFdelend \DIFaddbegin \DIFadd{$\tilde{\rho}_i^N,\,i\in\{1,\dots,n\}$ }\DIFaddend for randomly chosen \DIFdelbegin \DIFdel{$\rho_i \in \mathbb{Z}_N$ and return
        }\DIFdelend \DIFaddbegin \DIFadd{$\tilde{\rho}_i \in \mathbb{Z}_N$ and return
        }\DIFaddend \begin{equation*}
            \left\langle(N+1)^{\sum^m_{j=1}a^{(t^*)(b)}_{i,j}\omega^{(t^*)}_j}\DIFdelbegin \DIFdel{\rho}\DIFdelend \DIFaddbegin \tilde{\rho}\DIFaddend _i^N H(t^*)^{sk_i}\,\middle|\,i\in U\right\rangle
        \end{equation*}
        to $\mathcal{A}$.
...
%DIF < \vspace{-1\baselineskip}
\DIFaddbegin \vspace{-2\baselineskip}
\DIFaddend \begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{images/marko_ristic}}]{Marko Ristic}
...
%DIF < \vspace{-1\baselineskip}
\DIFaddbegin \vspace{-2\baselineskip}
\DIFaddend \begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{images/benjamin_noack}}]{Benjamin Noack}
is a professor of Computer Science at the Otto von Guericke University Magdeburg in Germany and head of the Autonomous Multisensor Systems (AMS) group. He received his diploma in computer science from the Karlsruhe Institute of Technology (KIT), Germany, in 2009. Afterward, he obtained his Ph.D. in 2013 at the Intelligent \DIFdelbegin \DIFdel{SensorActuator-Systems }\DIFdelend \DIFaddbegin \DIFadd{Sensor-Actuator-Systems }\DIFaddend Laboratory, Karlsruhe Institute of Technology (KIT), Germany. His research interests are in the areas of multi-sensor data fusion, distributed and decentralized Kalman filtering, combined stochastic and set-membership approaches to state estimation, and event-based systems.
\end{IEEEbiography}

%DIF < \vspace{-1\baselineskip}
\DIFaddbegin \vspace{-2\baselineskip}
\DIFaddend \begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{images/uwe_d_hanebeck}}]{Uwe D. Hanebeck}
is a chaired professor of Computer Science at the Karlsruhe Institute of Technology (KIT) in Germany and director of the Intelligent \DIFdelbegin \DIFdel{SensorActuator-Systems }\DIFdelend \DIFaddbegin \DIFadd{Sensor-Actuator-Systems }\DIFaddend Laboratory (ISAS). He obtained his Ph.D. degree in 1997 and his habilitation degree in 2003, both in Electrical Engineering from the Technical University in Munich, Germany. His research interests are in the areas of information fusion, nonlinear state estimation, stochastic modeling, system identification, and control with a strong emphasis on theory-driven approaches based on stochastic system theory and uncertainty models. He is author and coauthor of more than 500 publications in various high-ranking journals and conferences and an IEEE Fellow.
\end{IEEEbiography}
