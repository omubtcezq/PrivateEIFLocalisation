...
We're sorry that the introduction was found too long and agree that it could be made shorter. We have now greatly reduced the length of the introduction and removed a lot of the cryptographic reminders throughout the work. We hope that the new layout is easier to read and better suited to the target audience of the TAC journal.
...
We're glad to hear that the contribution was found interesting and important. We have made a very large revision of the work, including rewriting, reorganising and shortening the text to make the information easier to follow and better suited to the audience of the TAC journal.
...
% The method proposed achieves the desired security propoerties by modifying the range-only measurement model and rearranging the terms in the Extended Information Filter (EIF) during the filter update step. With this method, using the EIF is cruicial for reducing the communication of distributed participants to that which can use the introduced encryption scheme, and thus meet the security properties. 

% While a particle filter that can reduce its communication to suit another type of encryption scheme may exist, it is beyond the scope of this work and would likely differ both in its required encryption scheme and in any filter or model modifications.

\item \reviewtext{2) How does the linear combination reflect the dynamics of the investigated system in Eq.(2)?}
  
% TODO complete
%The linear combination aggregation scheme is only used to encrypt communication required during computation of the EIF update step. Ideally, the encryption has no effect on the proposed filter, but in practice, minor descrepancies are introduced with the quantisation of floating-piont numbers used for encryption and homomorphic summation. The proposed filter itself requires a modified measurement function to be able to use this scheme, which is where differences in performance with a standard EIF are introduced. The system model given in ($2$) is, however, unaffected in our method, in that both it and the standard EIF assume the measured state to follow these dynamics exactly.
...
% TODO complete
%Thank you for pointing this out. You are right to say that the Gaussianity of additive system noise is not required by either the standard EIF or our approach and has now been removed from the manuscript.
...
% TODO complete
%As we consider a linearising state estimation filter (the EKF), system noise covariance $\mat{Q}$ is 
%Estimating the system noise covariance $\mat{Q}$ is common practice when using linearising state estimation filters such as the EKF, and a requirement for predicting a
...
We agree that the results section was lacking and have rewritten it to try and make the evaluation clearer. To validate the accuracy of the algorithm, it has been compared to the industry standard EKF (reformulated as the EIF) and the comparison figure changed to make it clearer how the proposed filter performs. The computational performance is more difficult to compare. No alternative encryption scheme suitable for our filter (i.e. scheme meeting LCAO) exists, beyond the one introduced in the work. 

%A possible comparison, as remarked earlier in the paper, is using a scheme meeting AO combined with a unidirectional communication protocol, but we consider this beyond the scope of the work due to no existing 
...
We apologies for the mistake and have now corrected it, along with others throughout the work.
