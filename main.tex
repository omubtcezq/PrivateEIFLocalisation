...
\subsection{Notation}
Throughout this work, we will use the following notation. Lowercase underlined characters are vectors, $\vec{a}$, and random variables are bold, $\rv{a}$. Uppercase bold characters are matrices, $\mat{A}$, with $\mat{A}^{-1}$ denoting the inverse and $\mat{A}^\top$ the transpose. $|a|$ is the absolute value of $a$ and $\lVert\vec{a}\rVert$ the vector norm. Subscripts of the form $\rv{x}_{k|j}$ will denote an estimate of $\rv{x}_k$ using measurements from times up to and including $j$. $\mathsf{E}[\cdot]$ and $\mathsf{Var}[\cdot]$ are used for the expected value and variance, respectively, and encryption and decryption with key $k$ are denoted $\mathcal{E}_{k}(\cdot)$ and $\mathcal{D}_{k}(\cdot)$. Additionally, key $k$ may be ommited when inferrable from context. $\lfloor\cdot\rceil$ rounds to the nearest integer, $a\|b$ stands for a bitwise concatenation of $a$ and $b$ and sets will be written as $\{\dots\}$ while ordered sequences as $\langle\dots\rangle$.
...
\begin{equation}
    \vec{x}_{k+1|k} = \vec{f}(\vec{x}_{k|k}) \quad\textrm{and}\quad \mat{P}_{k+1|k} = \mat{F}_k\mat{P}_{k|k}\mat{F}_k^\top + \mat{Q}_k\,,
\end{equation} \label{eqn:ekf_prediction}
with Jacobian
\begin{equation}
...
The first thing we note when wanting to decompose $\vec{i}_{k,i}$ and $\mat{I}_{k,i}$ to a linear combination of functions of $\vec{x}_{k|k-1}$, is that $h_i$ cannot be rearranged in this way due to the present square-root. Similarly, the Jacobian of $h_i$ at $\vec{x}_{k|k-1}$,
\begin{equation}
    \mat{H}_{k,i} = 
    \begin{bmatrix}
...
\end{equation}
Here, $h'_i$ and $\mat{H}'_{k,i}$ are linear combinations of $x_{k|k-1}^2$, $y_{k|k-1}^2$, $x_{k|k-1}$ and $y_{k|k-1}$. To show how $\rvec{i}'_{k,i}$ and $\mat{I}'_{k,i}$ can be similarly rearranged, we also require the existence of measurements following a modified measurement model of the form
\begin{equation}
...
The approximation of $z'_{k,i}$ and its variance $\mathsf{Var}[v'_{k,i}]$ from the original measurements $z_{k,i}$ are complicated by the noise term $v_{k,i} \sim \mathcal{N}(0, r_i)$ in \eqref{eqn:measurement_model}. Squaring the original range measurements produces
\begin{equation}
    \begin{split}
    ...
    \end{split}
\end{equation}
with a new noise term $2h_i(\vec{x}_k)v_{k,i} + v_{k,i}^2$, now dependent on the measurement function $h_i$, and no longer zero-mean or Gaussian. We can compute the mean of this new noise term (a function of the Gaussian term $v_{k,i}$) as
\begin{equation}
...
\end{equation}
and is also dependent on $h_i(\vec{x}_k)$. To use the modified measurement \eqref{eqn:modified_measurement} with the EIF, we require an estimate for $\mathsf{Var}[v'_{k,i}]$ at the sensor as well. Additionally, a conservative estimate (\textit{i.e.}, a larger variance resulting in less confidence in measurements) is desirable to reduce filter divergence. The naive approach, replacing $h_i(\vec{x}_k)$ with $z_{k,i}$, only provides a conservative estimate when
\begin{equation}
    \begin{split}
        &z_{k,i} > h_i(\vec{x}_k) \\
        \implies &v_{k,i} > 0\,,
    \end{split}
\end{equation}
which cannot be guaranteed. Instead, we provide a conservative estimate with $95\%$ confidence due to the Gaussianity of $v_{k,i}$, by adjusting the replacement term $z_{k,i}$ by two of its standard deviations $\sqrt{r_i}$. The modified measurement's variance at time $k$ is therefore conservatively approximated by
\begin{equation}
    \begin{split}
        r_{k, i} &= 4(z_{k,i} + 2\sqrt{r_i})^2r_i + 2r_i^2 \\
        &\gtrapprox \Var{v'_{k,i}}
    \end{split}\,. \label{eqn:modified_measurement_variance_estimate}
\end{equation}
for each sensor $i$.

The existence of modified measurements $z'_{k,i}$ and their estimated variances $r_{k, i}$ are sufficient for sensors to compute measurement vector $\vec{i}_{k,i}$ and matrix $\mat{I}_{k,i}$ in the required form for linear-combination aggregation.
...
Now that range measurements can be modified by \eqref{eqn:modified_measurement}, and the modified model \eqref{eqn:modified_measurement_model} can be used, corresponding modified measurement vectors $\vec{i}'_{k,i}$ and matrices $\mat{I}'_{k,i}$ can be computed as linear combinations of powers of location elements. This is given by
\begin{equation}
    \begin{split}
...
\begin{remark}
    The above has been derived for two-dimensional localisation but can be similarly derived for three-dimensional case. However, we note that the number of weights increases combinatorially with the number of dimensions, thus affecting the cost of communication as well.
\end{remark}

The final step required for computing \eqref{eqn:hrz_linear_comb} and \eqref{eqn:hrh_linear_comb} at the sensors and \eqref{eqn:eif_modified_vec_update} and \eqref{eqn:eif_modified_mat_update} at the navigator using the proposed LCAO scheme, is handling the unique instance variables $t$ in the context of estimation timesteps $k$. This is done by elementwise linear combination of \eqref{eqn:hrz_linear_comb} and \eqref{eqn:hrh_linear_comb} and setting $t$ to the concatenation
\begin{equation}
    t=k\mathbin\|v\mathbin\|w\mathbin\|\tau
\end{equation}
for elements in row $v$ and column $w$, with $\tau=0$ for those in $\rvec{i}'_{k,i}$ and $\tau=1$ otherwise.
...
Measurement modification, real number encoding and linear combination aggregation are all required to compute the modified EIF from the previous section in a privacy-preserving manner. In this section, we summarise this entire localisation process and give the pseudocode for its execution. For brevity, we will assume $\phi$ and $M=N$ from section \ref{subsec:encoding} to be public information, and simplify the encoding notation $\mathsf{E}_{N,\phi,d}(\cdot)$ to $\mathsf{E}_{d}(\cdot)$. The privacy-preserving localization filter consists of the following steps.
\begin{LaTeXdescription}
...
\begin{algorithmic}[1]
    \Procedure{Prediction}{$\vec{x}_{k-1|k-1}$, $\mat{P}_{k-1|k-1}$, $\vec{f}$, $\mat{Q}$, $N$}
...
\begin{algorithmic}[1]
    \Procedure{Update}{$\vec{x}_{k|k-1}$, $\mat{P}_{k|k-1}$, $N$, $\lambda$}
...
\begin{tikzpicture}[font=\footnotesize,scale=0.95]
    ...
    \node at (3.25,5.875) {$\vec{x}_{k|k-1}$, $\mat{P}_{k|k-1}$};
    ...
    \node at (3.25,4.5) {$\left\{\mathcal{E}_{pk_0}(\mathsf{E}_0(\rv{\omega})) \,|\, \rv{\omega} \!\in\! \{\rv{x}^3_{k|k-1}, \rv{y}^3_{k|k-1}, \dots\}\right\}$};
    ...
    \node at (1.25,-3.125) {$z'_{k,1}$, $r_{k,1}$};
    \node at (5.25,-3.125) {$z'_{k,n}$, $r_{k,n}$};
    ...
    \node at (3.25,-7.125) {$\vec{x}_{k|k}$, $\mat{P}_{k|k}$};
\end{tikzpicture}
