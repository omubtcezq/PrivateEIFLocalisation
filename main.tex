...
\begin{description}
    \item[Multi-Client Function Encryption] This type of encryption allows the computing of a specific function given encrypted inputs []. It is suitable for privacy-preserving distributed data as encryptions must be combined before a result is obtained, and can be considered a generalisation of the aggregation schemes described earlier. While such schemes that can compute arbitrary functions are still too computationally expensive for practical use [], partial schemes supporting only some functions, such as computing vector dot products [], exist, but have yet to find applications in more complicated signal processing tasks.
\end{description}
...
The recursive nature of the problem definition above complicated the definition and proof of security requirements.

We will instead decompose the communication part of the problem into a form of aggregation, which we can reason and prove more easily.

Due to the non-linearity of the measurement model \eqref{eqn:measurement_model}, we will use a linearisation 

This type of linearisation will depend on the current state estimate and therefore requires communication between measurement producers and state estimators, in our case communication between the sensors and navigator

The basic idea of our aggregation decomposition is to formulate the computing of model linearisation with the ability to compute weighted sums of state-related information at sensors, and again summing



% note also the reason for not computing sums at the navigator (will require j sums to be sent rather than n)
% Possible as a remarkat the end? "We note that y= ... = ... and argue the benefit of this method depends on j >> n reducing communication costs
...
\subsection{Our Scheme} \label{sec:our_scheme}
