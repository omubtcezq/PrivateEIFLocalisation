...
\begin{equation}
    \rv{z}_k = h_i(\rvec{x}_k)+\rv{v}_k\,, \label{eqn:measurement_model}
\end{equation}
with noise $\rv{v}_k \sim \mathcal{N}(0,r_i)$, while the measurement function $h_i$, for sensor $i$ at location
\begin{equation}
    \vec{s}_i = 
    \begin{bmatrix}
        s_{x,i} & s_{y,i}
...
Normal privacy-preserving aggregation encryption of $\rvec{i}_{k,i}$ and $\mat{I}_{k,i}$ at the sensors would require the navigator to disclose some location information in $\rvec{x}_k$, for the computing of Jacobians $\mat{H}_{k,i}$. Instead, we want to formulate $\rvec{i}_{k,i}$ and $\mat{I}_{k,i}$ such that the sensors can use an LCAO secure encryption scheme, with location information in $\rvec{x}_k$ as weights from the navigator. As was shown in Section \ref{sec:our_scheme}, this guarantees that sensors cannot learn navigator location information $\rvec{x}_k$, and the navigator cannot learn individual sensor values $\rvec{i}_{k,i}$ and $\mat{I}_{k,i}$. From \eqref{eqn:measurement_vec} and \eqref{eqn:measurement_mat} we see that such a reformulation sets requirements on the possible measurement functions $h_i$. As matrix multiplication and scalar multiplication both preserve linearity, it is sufficient for $h_i$ and $\mat{H}_{k,i}$ to be linear combinations of information private to the navigator, for $\rvec{i}_{k,i}$ and $\mat{I}_{k,i}$ to be as well. Next, we will show how this can be achieved for the specific case of two-dimensional range-only localisation.
...
Recalling the state definition \eqref{eqn:state_def} and two-dimensional measurement model \eqref{eqn:measurement_model}, we see that $h_i$ cannot be rearranged to a linear combination of elements private to the navigator. In addition, the Jacobian of $h_i$,
\begin{equation}
    \mat{H}_{k,i} = 
    \begin{bmatrix}
    ...
    \end{bmatrix}\,,
\end{equation}
cannot be either. Instead, we consider the adjusted measurement functions
\begin{equation}
    h'_i(\rvec{x}_k) = h_i(\rvec{x}_k)^2\,. \label{eqn:adjusted_measurement_funcs}
\end{equation}
Measurement functions in this form allow rearrangement of the corresponding adjusted measurement vector $\rvec{i}'_{k,i}$ and measurement matrix $\mat{I}'_{k,i}$ into linear combinations of navigator information.The modified measurement functions $h'_i$ can be written as
\begin{equation}
    \begin{split}
        h'_i(\rvec{x}_k) &= \left\|
        \begin{bmatrix}
            \rv{x}_k & \rv{y}_k
        \end{bmatrix}^\top - \vec{s}_i\right\|^2 \\
...
As stated previously, computing $\rvec{i}'_{k,i}$ and $\mat{I}'_{k,i}$ will preserve this ability, albeit increasing the number of weights to include combinations of their products. $\rvec{i}'_{k,i}$ can now be written as
\begin{equation}
    \begin{split}
        \rvec{i}'_{k,i} &= \mat{H}_{k,i}^{\prime\top} \mean{r}_{k,i}^{-1}(\rv{z}'_{k,i} - h'_i(\rvec{x}_k) + \mat{H}'_{k,i}\rvec{x}_k) \\
        &= \\
        &\hspace{-1em}
        \begin{bmatrix}
            \begin{split}
                &(2\mean{r}_{k,i}^{-1})\rv{x}_k^3 + (2\mean{r}_{k,i}^{-1})\rv{x}_k\rv{y}_k^2 + (-\mean{r}_{k,i}^{-1}s_x)\rv{x}_k^2 \\
                &\quad+ (-2\mean{r}_{k,i}^{-1}s_x)\rv{y}_k^2 + (2\mean{r}_{k,i}^{-1}\rv{z}'_{k,i})\rv{x}_k + (-2\mean{r}_{k,i}^{-1}s_x^2)\rv{x}_k \\
                &\quad+ (-2\mean{r}_{k,i}^{-1}s_y^2)\rv{x}_k + (2\mean{r}_{k,i}^{-1}s_x^3) + (2\mean{r}_{k,i}^{-1}s_xs_y^2) \\
                &\quad+ (-2\mean{r}_{k,i}^{-1}s_x \rv{z}'_{k,i})
            \end{split}
            \vspace{5pt}\\
            \begin{split}
                &(2\mean{r}_{k,i}^{-1})\rv{y}_k^3 + (2\mean{r}_{k,i}^{-1})\rv{x}_k^2\rv{y}_k + (-2\mean{r}_{k,i}^{-1}s_y)\rv{x}_k^2 \\
                &\quad+ (-2\mean{r}_{k,i}^{-1}s_y)\rv{y}_k^2 + (2\mean{r}_{k,i}^{-1}\rv{z}'_{k,i})\rv{y}_k + (-2\mean{r}_{k,i}^{-1}s_x^2)\rv{y}_k \\
                &\quad+ (-2\mean{r}_{k,i}^{-1}s_y^2)\rv{y}_k + (2\mean{r}_{k,i}^{-1}s_ys_x^2) + (2\mean{r}_{k,i}^{-1}s_y^3) \\
                &\quad+ (-2\mean{r}_{k,i}^{-1}s_y\rv{z}'_{k,i})
            \end{split}
        \end{bmatrix}
    \end{split}
\end{equation}
...
\begin{equation}
    \mat{H}_{k,i}^{\prime\top} \mean{r}_{k,i}^{-1}\mat{H}'_{k,i} = 
    \begin{bmatrix}
        \alpha_{11} & \alpha_{12} \\
        \alpha_{21} & \alpha_{22}
    \end{bmatrix}\,,
...
with
\begin{align}
    \alpha_{11} &= (4\mean{r}_{k,i}^{-1})\rv{x}_k^2 + (-8\mean{r}_{k,i}^{-1}s_x)\rv{x}_k + (4\mean{r}_{k,i}^{-1}s_x^2) \\
    \begin{split}
        \alpha_{12} &= (4\mean{r}_{k,i}^{-1})\rv{x}_k\rv{y}_k + (-4\mean{r}_{k,i}^{-1}s_y)\rv{x}_k + (-4\mean{r}_{k,i}^{-1}s_x)\rv{y}_k \\
        &\qquad+ (4\mean{r}_{k,i}^{-1}s_xs_y)
    \end{split}\\
    \alpha_{21} &= \alpha_{12} \\
    \alpha_{22} &= (4\mean{r}_{k,i}^{-1})\rv{y}_k^2 + (-8\mean{r}_{k,i}^{-1}s_y)\rv{y}_k + (4\mean{r}_{k,i}^{-1}s_y^2)\,.
\end{align}




Modification of measurements $z_{k,i}$ to suit the adjusted measurement functions will not preserve the Gaussian nature of the true measurement noise, and will additionally require estimation of the sensor measurement variance $r_i$. We will denote the modified measurement from sensor $i$ at time $k$ as $\rv{z}'_{k,i}$, and the estimated, now time-dependant, sensor variance as $\mean{r}_{k,i}$. The computations of these values will be discussed in Section \ref{subsec:range_measurement_mod}.
