...
\subsection{Notation}
Notation. Reminder to include symbols for: rounding, concatenation, ``divides'', vector norm, vector indexing (also note starts at 0)
...
The localisation scenario we consider in this work is that of model-based self-navigation using range-only sensors. We consider localisation in the two-dimensional case for simplicty, but will derive methods suitable for an extension to the three-dimensional equivalent.
...
\begin{equation}
    \mean{x}_k = f(\mean{x}_{k-1})+\rvec{w}_k\,, \label{eqn:process_model}
\end{equation}
...
\begin{equation}
    z_k = h_i(\mean{x}_k)+\rv{v}_k\,, \label{eqn:measurement_model}
\end{equation}
with noise $\rv{v}_k \sim \mathcal{N}(\vec{0},\mat{R})$, while the measurement function $h_i$, for sensor $i$ at location
\begin{equation}
    \vec{s}_i = 
    \begin{bmatrix}
        x_{i} & y_{i}
...
\begin{equation}
    \begin{split}
        ...
        \end{bmatrix}
        - \vec{s}_{i}\right\| \\
        &= \sqrt{(\mean{x}_{k[0]}-x_{i})^2 + (\mean{x}_{k[2]}-y_{i})^2}\,.
    \end{split}
\end{equation}
We wish to run a state estimation filter with models \eqref{eqn:process_model} and \eqref{eqn:measurement_model} such that all involved sensors $1 \leq i \leq n$ do not learn the estimated state $\mean{x}_k$ and the navigator does not learn sensor locations $\vec{s}_i,\,1 \leq i \leq n$ or their measurements $z_k$ at any time $k$.
...
\begin{description}
    ...
    \item[Consistent navigator broadcast] Received information is always equal. 
    \item[Honest-but-curious sensors] We adopt the honest-but-curious attacker model for sensors in our localisation method. That is, sensors will be assumed to follow the derived procedure correctly, but may use any gained sensitive information for other purposes. 
    \item[Computational capabilities] The computational requirements of the navigator and sensors are predominantly defined by encryption, decryption and homomorphic operations. We make the implicit assumption that all involved parties are computationally capable of computing the described encryption scheme and filter.
\end{description}
% In the context of wireless communication, such as aircraft locations, commonly used non-directional antennas are such that this assumption is reasonable. While hardware-specific methods may exist for broadcasting different information to individual sensors, we will consider this beyond the scope of our work.

% This is an inherent problem in estimation theory, often requiring inconsistency or outlier detection by the estimator. When keeping individual sensor measurements private, it can complicate this detection. For this reason, we exclude the possibility of Byzantine-failing sensors to simplify our construction and leave this for future work.
