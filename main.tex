...
\begin{align}
    ...
    \mathcal{D}(\mathcal{E}(m_1)^{m_2}\!\pmod{N^2}) &= m_1m_2\!\pmod{N}\,. \label{eqn:paillier_hom_mult}
\end{align}
...
\begin{description}
    ...
    \item[$\mathsf{CombEnc}(t, pk_0, sk_i, \mathcal{E}_{pk_0}(\omega_1),\dots,\mathcal{E}_{pk_0}(\omega_m), x_{i,1},\dots,x_{i,m})$] The linear combination encryption step at time $t$ is computed as 
    \begin{equation}
    ...
    \begin{equation*}
        ...
        \end{split}
    \end{equation*}
    for some values $r_i,r_j \in \mathbb{Z}_N\,,1\leq j \leq m$.

    \item[$\mathsf{AggDec}(t, pk_0, sk_0, y_1,\dots,y_n)$] Aggregation is computed as $y^{(t)} = \prod^n_{i=1}y^{(t)}_i \pmod{N^2}$, removing aggregation noise terms, and followed by Paillier decryption
    \begin{equation}
        \begin{split}
...
To show our scheme meets the security notion of LCAO, we prove by contrapositive that given a polynomial-time adversary $\mathcal{A}$ with non-negligible advantage $\mathsf{Adv}^{LCAO}(\mathcal{A})$ against a challenger using our scheme (\textit{i.e.} if we assume our scheme is not LCAO secure) we can construct an adversary $\mathcal{A}'$ with non-negligible advantage $\mathsf{Adv}^{AO}(\mathcal{A}')$ against a challenger using the Joye-Libert private aggregation scheme (\textit{i.e.} then the Joye-Libert scheme is not AO secure). As this is a contradiction to the Joye-Libert AO proof in \cite{}, our scheme must meet LCAO security.
\begin{pf}
    Consider adversary $\mathcal{A}$ playing the LCAO game defined in Section \ref{subsec:lcao}. The following is a construction of adversary $\mathcal{A}'$ playing the AO game from Appendix \ref{app:ao} against a challenger $\mathcal{C}$.
    \begin{description}
        \item[$\mathsf{Setup}$] $\mathcal{A}'$ receives $N$ and $H$ as public parameters from $\mathcal{C}$, and generates a new key with same size $N'=pq$ and creates aggregator public and private keys $pk_0$, $sk_0$, chooses an $m>1$, and gives $H$, $m$, and $pk_0$ to $\mathcal{A}$
        \item[$\mathsf{CombEnc}$] When $\mathcal{A}$ makes combine encryption queries $(i, t, \mathcal{E}_{pk_0}(\omega_1),\dots,\mathcal{E}_{pk_0}(\omega_m), x_{i,1},\dots,x_{i,m})$, $\mathcal{A}'$ uses $sk_0$ to decrypt weights $\omega_1,\dots,\omega_m$ and makes an $\mathsf{Enc}(i, t, \sum^m_{j=1}x_{i,j}\omega_j)$ query to $\mathcal{C}$. The received response is of the form $(N+1)^{\sum^m_{j=1}x_{i,j}\omega_j}H(t)^{sk_i}$, from which $H(t)^{sk_i}$ is recovered by
        \begin{equation*}
            H(t)^{sk_i} = (N+1)^{\sum^m_{j=1}x_{i,j}\omega_j}H(t)^{sk_i}((N+1)^{\sum^m_{j=1}x_{i,j}})^{-1} \pmod{N^2}
        \end{equation*}
        where $^-1$ denotes the modular inverse. 
    \end{description}

    Note that decryption and encryption are polynomial in security parameter and therefore\dots
\end{pf}
