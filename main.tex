...
\subsection{Notation}
Notation. Reminder to include rounding symbol
...
The localisation filter we introduce requires real-valued inputs and functions, and relies on a non-linear measurement model. We make use of an existing real-number encoding scheme which supports the homomorphic operations we'll require and an algebraic reformulation of the Extended Kalman Filter which reduces the filter update step to only use these operations.
...
% TODO remove when not needed - for its simlicity, and applicability to our multiplication and bound requirements, is described in \cite{farokhiSecurePrivateControl2017}. It provides any number of homomorphic additions, and a predefined number of homomorphic multiplications.


In the encryption scheme introduced, weights and values are limited to integer values, and operations computed $\pmod{N}$ bound meaningful inputs to $x \in \mathbb{Z}_N$ for any weight or value $x$. The encoding scheme we have used 




The encoding scheme is defined by the quantisation parameter $f$, representing the number of bits used to quantise the fractional part of a real number, and $N$ the modulus to use for handling negative numbers.

Encoding of a real number $a \in \mathbb{R}$ to an integer $e \in \mathbb{Z}$ is computed by
\begin{equation}
    e = \left\lfloor 2^{(m+1)f} a \right\rceil \pmod{N}\,,
\end{equation}
and decoding after $m$ homomorphic multiplications is computed by
\begin{equation}
    a = 
    \begin{dcases}
        \frac{e\hspace{-7pt}\pmod{N}}{2^{(m+1)f}}, &e\hspace{-7pt}\pmod{N} \leq \left\lfloor\frac{N}{2}\right\rfloor \\
        -\frac{N - e\hspace{-7pt}\pmod{N}}{2^{(m+1)f}}, &e\hspace{-7pt}\pmod{N} > \left\lfloor\frac{N}{2}\right\rfloor \\
    \end{dcases}\,.
\end{equation}

Note that adding encoding number to already encoded number requires the number of multiplications to be matched

that for the largest number of multiplications $m$, parameters for the encoding scheme when used with our ecncryption scheme should be defined such that
\begin{equation*}
    something \leq something\,else
\end{equation*}

encodings grow in size exponentially for each multilpication.
