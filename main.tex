...
The formal security game for IND-CPA has been given in Appendix \ref{app:ind-cpa}.
\end{definition}
...
\begin{description}
    \item[$\mathsf{Setup}(\kappa)$] On input of security paramater $\kappa$, generate the aggregator's public and private keys $pk_0$ and $sk_0$, and the remaining user private keys $sk_i,\,1\leq i \leq n$.
...
\begin{description}
    \item[Setup] The challenger runs the $\mathsf{Setup}$ algorithm and gives $pk_0$ to the attacker
