...
\begin{description}
    ...
    \item[Partially Homomorphic Encryption] A simplification of fully homomorphic schemes, these schemes provide only a subset, typically one, of algebraic operations on encrypted data []. The reduced computational requirements and implementation simplicty have made partially homomorphic encryption schemes commonly used in privacy-preserving signal processing tasks, including private matrix multilpication [], set intersection computation [] and control input aggregation []. However, these works are relatively restricted in application due to the limited operations provided.
\end{description}

% TODO Never talked about FE and it's distributed extensio MCFE. General solutions still too computationally infeasible, while very specific functions do exists - dot product
% "several novel methods for privacy preserving computations ...
% which have as of yet found little application in estimation and localisation tasks


Several of the aforementioned encryption schemes have found uses in privacy-preserving localisation, estimation, and control.
...
% TODO add overview section here - leads into crypto
% can use the first part of the current crypto problem in here - then go straight into the scheme there
...
\begin{tikzpicture}[font=\footnotesize,scale=0.9375]
    % Prediction
    \draw [gray] (0,1.75) rectangle (8,8);
    \node at (4,7.5) {1. Prediction};
    ...
    \node at (4,4.5) {$\langle\mathcal{E}_{pk_0}(\mathsf{E}_0(\rv{\omega})): \rv{\omega} \in \{\rv{x}^3_k, \rv{y}^3_k, \rv{x}^2_k\rv{y}_k, \dots , \rv{x}_k, \rv{y}_k\}\rangle$};
    
    
    % Measurement
    \draw [gray] (0,-4.75) rectangle (8,1.5);
    \node at (4,1) {2. Measurement};
    ...
    \fill [lightgray]  (0.5,-1.25) rectangle (3.5,-2.25);
    \node[align=center] at (2,-1.75) {$\mathcal{E}_{pk_0,sk_1}(\mathsf{E}_1(\rvec{i}'_{k,1}))$,\\$\mathcal{E}_{pk_0,sk_1}(\mathsf{E}_1(\mat{I}'_{k,1}))$};
    \fill [lightgray]  (4.5,-1.25) rectangle (7.5,-2.25);
    \node[align=center] at (6,-1.75) {$\mathcal{E}_{pk_0,sk_n}(\mathsf{E}_1(\rvec{i}'_{k,n}))$,\\$\mathcal{E}_{pk_0,sk_n}(\mathsf{E}_1(\mat{I}'_{k,n}))$};
    
    % Update
    \draw [gray] (0,-10.25) rectangle (8,-5);
    \node at (4,-5.5) {3. Update};
