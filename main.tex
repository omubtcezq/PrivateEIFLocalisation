...
Our proposed solution to this problem, is squaring sensor measurements used for the filter.
\begin{equation*}
    \begin{split}
        z^2 &= (h(x) + n)^2 \\
        &= h(x)^2 + 2nh(x) + n^2
    \end{split}\,,
\end{equation*}
where noise term $2nh(x) + n^2$ has mean $\sigma^2$ and standard deviation $2(2h(x)^2\sigma^2 + \sigma^4)$. We account for the non-zero mean by offsetting the the squared measurement and taking this as the adjust measurement model $h'(x) = h(x)^2$, and
\begin{equation*}
    \begin{split}
    ...
    \end{split}\,,
\end{equation*}
with variance approximated as $2(2z^2\sigma^2 + \sigma^4)$, which uses the original measurement $z$ as an unbiased estimate for $h(x)$.

While the above transformation provides a measurement model that no longer has Gaussian noise, it allows for the linear formulation required for the encryption scheme, and privacy of $H^{\prime\top} R^{\prime -1}(z' - h(x) + Hx)$ and $H^{\prime\top} R^{\prime -1}H'$. The adjusted measurement function is now given by
\begin{equation*}
    \begin{split}
...
The above formulation allows the use of our encryption scheme given to privately aggregate measurement matrix and vector above. Scheme encrypted weights $x$, $x^2$, $x^3$, $y$, $xy$, $x^2y$, $y^2$, $xy^2$ and $y^3$ are computed at each time step at the navigator, and each element of the measurement matrix and vector above is computed as a linear combination and prepared for aggregation at the navigator. 
