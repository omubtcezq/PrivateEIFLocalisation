...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        \Procedure{Prediction}{$\rvec{x}_{k-1}$, $\mat{P}_{k-1}$, $\vec{f}$, $\mat{Q}$, $N$, $f$}
        ...
        \For{$\rv{\omega}$ in $\langle\rv{x}_k^3,\rv{y}_k^3,\rv{x}_k^2\rv{y}_k,\rv{x}_k\rv{y}_k^2,\rv{x}_k^2,\rv{y}_k^2,\rv{x}_k,\rv{y}_k\rangle$} 
            \State Compute $\rv{\omega}' = \mathsf{E}_{N,f,0}(\rv{\omega})$ by \eqref{eqn:encode}
            \State $r \gets$ \Call{RandInt}{$1,N$}
            \State $\mathcal{E}_{pk}(\rv{\omega}') \gets (N+1)^{\rv{\omega}'}r^N \pmod{N^2}$
            \State Broadcast $\mathcal{E}_{pk}(\rv{\omega}')$ to sensors
        \EndFor
...
\begin{algorithm}
    \caption{Sensor Measurement at Sensor $i$}\label{alg:measurement_info}
    \begin{algorithmic}[1]
        \Procedure{Measurement}{$i$, $s_x$, $s_y$, $r_i$, $N$, $H$, $f$}
        \State Measure $\rv{z}_{k,i}$
        \While{Navigator broadcasting}
        \State Recieve $\mathcal{E}_{pk}((\rv{x}^3_k)') = \mathcal{E}_{pk}(\mathsf{E}_{N,f,0}(\rv{x}^3_k))$
        \State Recieve remaining weights in the above form
        \EndWhile
        \For{$i = 1$ to $2$} 
            \State $\beta_i \gets $
            \State Send $\beta_i$ to navigator
        \EndFor
        \For{$i = 1$ to $2$}
            \For{$j = 1$ to $2$} 
                \State Compute $\omega' = \mathsf{E}_{N,f,0}(\omega)$ by \eqref{eqn:encode}
                \State $r \gets$ \Call{RandIntInRange}{1,N}
                \State $\mathcal{E}_{pk}(\omega') \gets (N+1)^{\omega'}r^N \pmod{N^2}$
                \State Send $\mathcal{E}_{pk}(\omega')$ to sensors
            \EndFor
        \EndFor
        \EndProcedure
    \end{algorithmic}
