...
\begin{equation}
    \mean{x}_k = \vec{f}(\mean{x}_{k-1})+\rvec{w}_k\,, \label{eqn:process_model}
\end{equation}
...
\begin{description}
    ...
    A problem caused by this requirement is apparent in the context of aircraft navigation. It may be the case that some sensors are not within range of a navigator broadcast. We loosely propose that this scenario can be handled by pre-defined sensor subsets during an initial step by a trusted party. This has been captured in Figure \ref{fig:sensor_subsets}.
    \item[Consistent navigator broadcast] Further, we make the assumption that broadcast information from the navigator is recieved equal by all sensors. This means the navigator may not send different information to individual sensors during a single timestep.
...
Given process and measurement models \eqref{eqn:process_model} and \eqref{eqn:measurement_model}, the Extended Information Filter (EIF) is a reformulation of the Extended Kalman Filter (EKF), such that the update equations are simpified to sums of measurement information from each sensor. This requires the conversion of state estimate $\mean{x}_k$ and estimate covariance $\mat{P}_k$ to the information vector and matrix forms
\begin{equation}
    \mean{y}_k = \mat{P}_k^{-1}\mean{x}_k
\end{equation}
and
\begin{equation}
    \mat{Y}_k = \mat{P}_k^{-1}\,.
\end{equation}
In this form, the update equations for $n$ measurements at time $k$ are given by
\begin{equation}
    \mat{Y}_k = \mat{Y}_{k-1} + \sum^n_{i=1}\mat{H}^\top_{k,i} \mat{R}^{-1}_i \mat{H}_{k,i}
\end{equation}
and
\begin{equation}
    \mean{y}_k = \mean{y}_{k-1} + \sum^n_{i=1}\mat{H}^\top_{k,i} \mat{R}^{-1}_i (z_{k,i} - h_i(\mean{x}_k) + \mat{H}_{k,i}\mean{x}_k)\,,
\end{equation}
with Jacobian
\begin{equation}
    \mat{H}_{k,i} = \left.\frac{\partial h_i}{\partial \vec{x}}\right|_{\mean{x}_k}
\end{equation}
for each sensor $1 \leq i \leq n$. 

Computing the prediction step of the filter can be done by converting the information vector and matrix back to state estimate and covariance with
\begin{equation}
    \mat{P}_k = \mat{Y}_k^{-1}
\end{equation}
and
\begin{equation}
    \mean{x}_k = \mat{P}_k\mean{y}_k\,,
\end{equation}
and using the normal EKF prediction equations
