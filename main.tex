...
\subsection{Notation}
Notation. Reminder to include symbols for: rounding, concatenation, ``divides''
...
Our proposed solution to this problem, is working with the adjust measurement function $h'(x) = h(x)^2$. The method for modifying a normal range-sensor measurment in this way and handling the no-longer Gaussian, distance-dependant variance of the measurement noise, will be discussed in Section \ref{subsec:range_measurement_mod}. While this transformation complicated the measurement model noise, it allows for the linear formulation required for the encryption scheme, and the privacy of $H^{\prime\top} R^{\prime -1}(z' - h(x) + Hx)$ and $H^{\prime\top} R^{\prime -1}H'$. The adjusted measurement function is now given by
\begin{equation*}
    \begin{split}
...
Lastly, since we aggregate multiple linear combinations at each time step (each element in the result matrix and vector comes from a seperate aggregation), we require unique hash values for each aggregation. This is handled by hashing $t \| i \| j$ rather than $t$ for each element $x_{i,j}$ when computing combined aggregation encryption in \eqref{eqn:our_scheme_lin_comb}.
...
\begin{equation*}
    ...
    \end{split}\,,
\end{equation*}
where noise term $2nh(x) + n^2$ has mean $\sigma^2$ and standard deviation $2(2h(x)^2\sigma^2 + \sigma^4)$. We account for the non-zero mean by offsetting the the squared measurement and taking this as the adjusted measurement model
\begin{equation*}
    \begin{split}
