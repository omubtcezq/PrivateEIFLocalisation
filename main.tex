...
Passive attacks only from sensors to learn navigator position (Otherwise could do some kind of attack that would send fake measurement and note the change in it's own measurements - possible this would give away average of other sensors' measurements but unclear). Justify by saying sensors need to behave for localisation to work in the first place, for example GPS.
...
Fixed sensor subsets of which only whole subsets can be used at once. Maybe a picture of what this might look like in a high level distributed localisation diagram.
