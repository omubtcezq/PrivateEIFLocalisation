...
\end{figure}
The simulation root mean square error (RMSE) of the privacy-preserving method has been compared to the RMSE of the standard EIF (algebraic equivalent of the EKF), using the unmodified measurements. Measurement and estimation in each layout from figure \ref{fig:sim_layouts} consists of $50$ filter iterations and was run $100$ times. Unmodified measurement variances were taken as $r_i=5$ and a large fractional precision factor, $\phi=2^{32}$, was chosen. The results can be seen in figure \ref{fig:sim_layout_errors}.
\begin{figure}[htbp]
    \centering
