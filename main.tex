...
To show how $\rvec{i}'_{k,i}$ and $\mat{I}'_{k,i}$ can be formulated in a similar manner using \eqref{eqn:matrix_mult_weights}, we denote the modified measurements as $\rv{z}'_{k,i}$, and modified measurement variances as $\mean{r}_{k,i}$. The derivation of these from the normal measurements and variances $\rv{z}_{k,i}$ and $r_i$ will be derived in Section \ref{subsec:range_measurement_mod}. The resulting linear combinations are given by
\begin{equation}
    \begin{split}
        ...
        \begin{bmatrix}
            \begin{split}
                ...
                &\quad+ (-2\mean{r}_{k,i}^{-1}s_x)\rv{y}_k^2 + (2\mean{r}_{k,i}^{-1}\rv{z}'_{k,i})\rv{x}_k \\
                &\quad+ (-2\mean{r}_{k,i}^{-1}s_x^2)\rv{x}_k + (-2\mean{r}_{k,i}^{-1}s_y^2)\rv{x}_k \\
                &\quad+ (2\mean{r}_{k,i}^{-1}s_x^3) + (2\mean{r}_{k,i}^{-1}s_xs_y^2) \\
            ...
            \begin{split}
                ...
                &\quad+ (-2\mean{r}_{k,i}^{-1}s_y)\rv{y}_k^2 + (2\mean{r}_{k,i}^{-1}\rv{z}'_{k,i})\rv{y}_k \\
                &\quad+ (-2\mean{r}_{k,i}^{-1}s_x^2)\rv{y}_k + (-2\mean{r}_{k,i}^{-1}s_y^2)\rv{y}_k \\
                &\quad+ (2\mean{r}_{k,i}^{-1}s_ys_x^2) + (2\mean{r}_{k,i}^{-1}s_y^3) \\
...
\begin{equation}
    \begin{split}
        \mat{I}'_{k,i} &= \mat{H}_{k,i}^{\prime\top} \mean{r}_{k,i}^{-1}\mat{H}'_{k,i} \\
        &=
        \begin{bmatrix}
            \alpha_{11} & \alpha_{12} \\
            \alpha_{21} & \alpha_{22}
...
\begin{rem}
    The solutions \eqref{eqn:hrz_linear_comb} and \eqref{eqn:hrh_linear_comb} have been derived specifically for two-dimensional localisation, but can be similarly computed for the three-dimensional case. It is worth noting however, the additional cost required for increasing localisation dimension. The number of weights is increased in the rearranged functions $h'_i$, and therefore combinatorially increased in $\rvec{i}'_{k,i}$ and $\mat{I}'_{k,i}$ by \eqref{eqn:matrix_mult_weights}. This in turn results in a combinatorial increase in required weights, and therefore communication, with respect to the number of state parameters required for computing functions $h_i$.
\end{rem}
...
In Section \ref{subsec:observation_model}, we assumed the existance of modified sensor measurements $\rv{z}'_{k,i}$ and measurement variances $\mean{r}_{k,i}$ such that we used a measurement model of the same form as \eqref{eqn:measurement_model} given the modified measurement function in \eqref{eqn:modified_measurement_func}.



--

% Modification of measurements $z_{k,i}$ to suit the modified measurement functions will not preserve the Gaussian nature of the true measurement noise, and will additionally require estimation of the sensor measurement variance $r_i$. We will denote the modified measurement from sensor $i$ at time $k$ as $\rv{z}'_{k,i}$, and the estimated, now time-dependant, sensor variance as $\mean{r}_{k,i}$. The computations of these values will be discussed in Section \ref{subsec:range_measurement_mod}.
...
\begin{equation*}
    ...
    \end{split}\,,
\end{equation*}
where noise term $2nh(x) + n^2$ has mean $\sigma^2$ and standard deviation $2(2h(x)^2\sigma^2 + \sigma^4)$. We account for the non-zero mean by offsetting the squared measurement and taking this as the modified measurement model
\begin{equation*}
    \begin{split}
