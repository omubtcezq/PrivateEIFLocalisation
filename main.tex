...
The problem we will consider is a model-based localisation estimation problem in the presence of $n$ range-only sensors. This is defined by the typical process model
\begin{equation}
    x_k = f(x_{k-1})+w_k
\end{equation}
...
Prediction step can be computed in either the EIF form or the equivalent EKF state and covariance can be computed and the prediction computed normally
...
Using the EIF and distance sensors, we see that the required computation of measurement vector and matrices is reduced to computing $H^\top R^{-1}(z - h(x) + Hx)$ and $H^\top R^{-1}H$, for one-dimensional $R$ and jacobian $H$, at each sensor. As this is requires current state information $x$, relevant attributes need to be sent to the sensors. 

We wish to encrypt current state information and use the encryption scheme defined to compute measurement information $H^\top R^{-1}(z - h(x) + Hx)$ and $H^\top R^{-1}H$ as a linear combination of state information, and encrypt it for aggregation. Thus allowing the navigator to only learn sums $\sum^n_{i=1}H^\top R^{-1}_i(z_i - h(x) + Hx)$ and $\sum^n_{i=1}H^\top R_i^{-1}H$ and update its position.
...
The requirement introduced above, is that $H^\top R^{-1}(z - h(x) + Hx)$ and $H^\top R^{-1}H$ can be computed as linear combinations of state-derived variables. Here $H$ is a Jacobian and includes current state information as well.

State-derived variables refer to state vector elements, as well as non-linear functions of these elements, which do not require sensor position or measurement information to be computed. As our encryption scheme described above, only supports linear combinations of weights, in this case state-derived variables, all non-linear functions required for measurements information to be computed, need to be treated as individual weights to be combined.

From the definition of matrix multiplication, we see that any $KxL$ matrix $D=ABC$, where $B$ has dimensions $MxN$, has element $d_{i,j}$ defined by
\begin{equation*}
    d_{i,j} = \sum^{N}_{n=1}\sum^{M}_{m=1}a_{i,m}b_{m,n}c_{n,j}\,,
\end{equation*}
and can therefore be considered as a linear combination of all elements $a_{i,m}c_{n,j}$. Applying this to the measurement matrix and vector $H^\top R^{-1}(z - h(x) + Hx)$ and $H^\top R^{-1}H$, we require that all values of $h(x)$, and all elements in Jacobian $H$ be linear combinations of state-derived variables

Next, we show how this can be achieved for the specific case of 2-dimensional localisation, with range-only sensors.
...
\subsection{Localisation Measurement Model}
...
\begin{equation*}
    ...
    \end{bmatrix}\,.
\end{equation*}
The problem with the above function $h$ and Jacobian $H$ is that they cannot be reformulated as a linear combination of weights, where weights are defined by functions of $x$ and $y$, and do not include $s_x$ and $s_y$.

Our proposed solution to this problem, is working with the adjust measurement function $h'(x) = h(x)^2$. The method for modifying a normal range-sensor measurement in this way and handling the no-longer Gaussian measurement noise with variance dependant on distance, will be discussed in Section \ref{subsec:range_measurement_mod}. While this transformation complicated the measurement model noise, it allows for the linear formulation required for the encryption scheme, and the privacy of $H^{\prime\top} R^{\prime -1}(z' - h(x) + Hx)$ and $H^{\prime\top} R^{\prime -1}H'$. The adjusted measurement function is now given by
\begin{equation*}
    \begin{split}
...
\begin{equation*}
    ...
    \end{bmatrix}\,.
\end{equation*}
The above function and jacobian elements are linear combinations of state-derived functions $x$, $x^2$, $y$, $xy$ and $y^2$. From our equation $\cdot$ in the previous section, we can now define $H^{\prime\top} R^{\prime -1}(z' - h(x) + Hx)$ and $H^{\prime\top} R^{\prime -1}H'$ in terms of state-derived functions $x$, $x^2$, $x^3$, $y$, $xy$, $x^2y$, $y^2$, $xy^2$ and $y^3$.
\begin{equation*}
    \begin{split}
...
The conversion of measurement function $h'(x) = h(x)^2$ is in practice complicated by gausian nature of the range sensor measurement noise. As we only know $z = h(x) + n$, where $n~\mathcal{N}(0,\sigma)$, we can estimate the new measurement $z'= z^2$. This results in the new measurement
\begin{equation*}
    \begin{split}
        z^2 &= (h(x) + n)^2 \\
        &= h(x)^2 + 2nh(x) + n^2 \\
...
The navigator, which may maliciously choose weights at every time step, is always restricted to learning at most the sum $\sum^n_{i=1} a_i$ for some values $a_i$ derived from sensor $i$ location and measurement. It is therefore intuitive to say that at most the navigator can learn the average of values kept private, that is, the navigator can at most learn the average position and location of sensors. The same can be said for any corrupted subset of honest but curious sensors and malicious navigator.
