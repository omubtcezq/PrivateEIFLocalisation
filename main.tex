...
\author{Marko~Ristic$^{1}$,
        Benjamin~Noack$^{2}$,~\IEEEmembership{Member,~IEEE,}
        and~Uwe~D.~Hanebeck$^{1}$,~\IEEEmembership{Fellow,~IEEE}% <-this % stops a space
\thanks{$^{1}$Marko Ristic and Uwe D. Hanebeck are with the Intelligent Sensor-Actuator-Systems Laboratory (ISAS), Institute for Anthropomatics, Karlsruhe Institute of Technology (KIT), Germany (e-mail: \{marko.ristic, uwe.hanebeck\}@kit.edu).}% <-this % stops a space
\thanks{$^{2}$Benjamin Noack is with the Autonomous Multisensor Systems Group (AMS), Institute for Intelligent Cooperating Systems (ICS), Otto von Guericke University Magdeburg (OVGU), Germany (e-mail: benjamin.noack@ovgu.de).}}% <-this % stops a space
...
In section \ref{sec:problem_statement}, we introduce both the cryptographic and estimation problems considered in this work, before giving some relevant preliminaries in section \ref{sec:preliminaries}. Section \ref{sec:lcao_scheme} proposes a cryptographic scheme meeting our desired security properties and section \ref{sec:priv_localization} gives a solution to the estimation problem making use of this scheme. Simulation results are discussed in section \ref{sec:sim_and_results} and concluding remarks are made in section \ref{sec:conclusion}.
...
At every \textit{instance} $t$ (used to distinguish from an estimation \textit{timestep}), the navigator first broadcasts $m$ weights $\omega_j^{(t)}, 1 \leq j \leq m$ to all sensors $1\leq i\leq n$, who individually compute linear combinations $l^{(t)}_i=\sum^m_{j=1}a_{j,i}^{(t)}\omega_i^{(t)}$ based on their measurement data $a_{j,i}$. Linear combinations are then sent back to the navigator, who computes their sum $\sum^n_{i=1}l^{(t)}_{i}$. This two-step linear combination aggregation protocol has been visually displayed in figure \ref{fig:agg_steps}.
\begin{figure}[htbp]
\centering
\begin{tikzpicture}[font=\footnotesize,scale=0.95]
    ...
    % Navigator
    \fill [green!80!black!60] (1.375,-3) rectangle (5.125,-1.25);
    ...
    \fill  (5.25,-5.25) [blue!80!black!40] ellipse (0.4 and 0.4);
    \node at (5.25,-5.875) {Sensor $n$};
    \fill  (1.25,-5.25) [blue!80!black!40] ellipse (0.4 and 0.4);
...
\end{equation}
with system noise covariance $\mathsf{Cov}[\vec{w}_k]=\mat{Q}_k$. Measurements of the system follow a measurement model dependent on sensor $i$, $0\leq i \leq n$, given by 
\begin{equation}
...
\end{LaTeXdescription}
Additionally, we note that in the above construction, all weights $\omega^{(t)}_j$ and values $a^{(t)}_{i,j}$ are integers and the resulting linear combinations and aggregation are computed modulo $N$. 
...
\end{equation}
for sensors $1 \leq i \leq n$. The filter's prediction step can be computed by converting information vector $\hat{\vec{y}}_{k|k}$ and matrix $\mat{Y}_{k|k}$ back to state estimate and covariance 
\begin{equation}
    \hat{\vec{x}}_{k|k} = \mat{Y}_{k|k}^{-1}\hat{\vec{y}}_{k|k} \quad\textrm{and}\quad \mat{P}_{k|k} = \mat{Y}_{k|k}^{-1}\,,
\end{equation}
before using the normal EKF prediction equations
\begin{equation}
    \hat{\vec{x}}_{k|k-1} \!=\! \vec{f}_k(\hat{\vec{x}}_{k-1|k-1}) \ \textrm{and}\ \mat{P}_{k|k-1} \!=\! \mat{F}_k\mat{P}_{k-1|k-1}\mat{F}_k^\top + \mat{Q}_k, \label{eqn:ekf_prediction}
\end{equation}
with Jacobian
\begin{equation}
    \mat{F}_{k} = \left.\frac{\partial \vec{f}_k}{\partial \vec{x}}\right|_{\hat{\vec{x}}_{k-1|k-1}}\,. \label{eqn:jacobian_f}
\end{equation}
...
\begin{equation}
    \mathsf{E}[2h_i(\vec{x}_k)v_{k,i} + v_{k,i}^2] = r_{k,i}\, \label{eqn:modified_noise_mean}
\end{equation}
...
\end{equation}
with now zero-mean noise $v'_{k,i} = 2h_i(\vec{x}_k)v_{k,i} + v_{k,i}^2 - r_{k,i}$. The noise in this case (again a function of Gaussian term $v_{k,i}$) has variance 
\begin{equation}
...
\begin{align*}
    \begin{split}
        \alpha_1^{(i)} &= (2r_{k,i}^{\prime-1})\hat{x}_{k|k-1}^3 + (2r_{k,i}^{\prime-1})\hat{x}_{k|k-1}\hat{y}_{k|k-1}^2 \\
    ...
    \end{split}\\
    \begin{split}
        \alpha_2^{(i)} &= (2r_{k,i}^{\prime-1})\hat{y}_{k|k-1}^3 + (2r_{k,i}^{\prime-1})\hat{x}_{k|k-1}^2\hat{y}_{k|k-1} \\
...
\begin{equation}
    \begin{split}
        ...
        &=
        \begin{bmatrix}
            \beta_1^{(i)} & 0 & \beta_2^{(i)} & 0 \\
            0 & 0 & 0 & 0 \\
            \beta_3^{(i)} & 0 & \beta_4^{(i)} & 0 \\
            0 & 0 & 0 & 0
        \end{bmatrix}\,,
...
with
\begin{align*}
    \beta_1^{(i)} &= (4r_{k,i}^{\prime-1})\hat{x}_{k|k-1}^2 + (-8r_{k,i}^{\prime-1}s_{x,i})\hat{x}_{k|k-1} + (4r_{k,i}^{\prime-1}s_{x,i}^2)\,, \\
    \begin{split}
        \beta_2^{(i)} &= (4r_{k,i}^{\prime-1})\hat{x}_{k|k-1}\hat{y}_{k|k-1} + (-4r_{k,i}^{\prime-1}s_{y,i})\hat{x}_{k|k-1}  \\
        &\qquad+ (-4r_{k,i}^{\prime-1}s_{x,i})\hat{y}_{k|k-1} + (4r_{k,i}^{\prime-1}s_{x,i}s_{y,i})\,,
    \end{split}\\
    \beta_3^{(i)} &= \beta_{12}\ \textrm{and} \\
    \beta_4^{(i)} &= (4r_{k,i}^{\prime-1})\hat{y}_{k|k-1}^2 + (-8r_{k,i}^{\prime-1}s_{y,i})\hat{y}_{k|k-1} + (4r_{k,i}^{\prime-1}s_{y,i}^2)\,.
\end{align*}
...
\begin{algorithmic}[1]
    \Procedure{Prediction}{$\hat{\vec{x}}_{k-1|k-1}$, $\mat{P}_{k-1|k-1}$, $\vec{f}_k$, $\mat{Q}_k$, $N$}

    \State Compute $\mat{F}_k$ by \eqref{eqn:jacobian_f}
    \State $\hat{\vec{x}}_{k|k-1} \gets \vec{f}_k(\hat{\vec{x}}_{k-1|k-1})$
...
\begin{algorithmic}[1]
    ...
    \State Let $\bm{\alpha}_{v}^{(i)}$ represent the encryption of $\alpha_{v}^{(i)}$ in \eqref{eqn:hrz_linear_comb}
    \State $\bm{\alpha}_{1}^{(i)} \gets \mathcal{E}_{pk_0}(\mathsf{E}_{0}(\hat{x}^3_{k|k-1}))^{\mathsf{E}_{0}(2r_{k,i}^{\prime-1})}\cdot$\par
    ...
    \State Compute $\bm{\alpha}_{2}^{(i)}$ using \eqref{eqn:hrz_linear_comb}, \eqref{eqn:our_scheme_lin_comb} and the remark from section \ref{sec:lcao_scheme} in the form above
    \For{$v \gets 1$ to $2$}
        \State Send $\bm{\alpha}_{v}^{(i)}$ to the navigator
    \EndFor

    \State Let $\bm{\beta}_{w}^{(i)}$ represent an encryption of $\beta_{w}^{(i)}$ in \eqref{eqn:hrh_linear_comb}
    \State $\bm{\beta}_{1}^{(i)} \gets \mathcal{E}_{pk_0}(\mathsf{E}_{0}(\hat{x}^2_{k|k-1}))^{\mathsf{E}_{0}(4r_{k,i}^{\prime-1})}\cdot$\par
    \ $\mathcal{E}_{pk_0}(\mathsf{E}_{0}(\hat{x}_{k|k-1}))^{\mathsf{E}_{0}(-8r_{k, i}^{\prime-1}s_{x,i})}\cdot$\par
    \ $(N+1)^{\mathsf{E}_{1}(4r_{k,i}^{\prime-1}s_{x,i}^2)}\cdot$\par
    \ $H(k\mathbin\|1\mathbin\|1\mathbin\|1)\pmod{N^2}$
    \State Compute remaining $\bm{\beta}_{w}^{(i)}$ using \eqref{eqn:hrh_linear_comb}, \eqref{eqn:our_scheme_lin_comb} and the remark from section \ref{sec:lcao_scheme} in the form above
    \For{$w \gets 1$ to $4$}
        \State Send $\bm{\beta}_{w}^{(i)}$ to the navigator
    \EndFor
    \EndProcedure
\end{algorithmic}
\end{algorithm}
...
\begin{algorithmic}[1]
    ...
    \For{$v \gets 1$ to $2$}
        \State Receive $\bm{\alpha}_{v}^{(i)}$ from each sensor $1\leq i \leq n$
    \EndFor

    \For{$w \gets 1$ to $4$}
        \State Receive $\bm{\beta}_{w}^{(i)}$ from each sensor $1\leq i \leq n$
    \EndFor

    \State Let $\bm{\alpha}_{v}$ represent an encryption of $\sum_{i=1}^n\alpha_{v}^{(i)}$
    \For{$v \gets 1$ to $2$}
        \State $\bm{\alpha}_{v} \gets \prod_{i=1}^n\bm{\alpha}_{v}^{(i)}$
        \State Compute $\mathcal{D}_{sk_0}(\bm{\alpha}_{v})$ with $\lambda$ by \eqref{eqn:our_scheme_decrypt}
        \State Compute $\mathsf{E}^{-1}_{1}(\mathcal{D}_{sk_0}(\bm{\alpha}_{v}))$ by \eqref{eqn:decode}
    \EndFor
    \State Construct $\sum_{i=1}^n\vec{i}'_{k,i}$ from decoded decryptions above

    \State Let $\bm{\beta}_{w}$ represent an encryption of $\sum_{i=1}^n\beta_{w}^{(i)}$
    \For{$w \gets 1$ to $4$}
            \State $\bm{\beta}_{w} \gets \prod_{i=1}^n\bm{\beta}_{w}^{(i)}$
            \State Compute $\mathcal{D}_{sk_0}(\bm{\beta}_{w})$ with $\lambda$ by \eqref{eqn:our_scheme_decrypt}
            \State Compute $\mathsf{E}^{-1}_{1}(\mathcal{D}_{sk_0}(\bm{\beta}_{w}))$ by \eqref{eqn:decode}
    \EndFor
...
\begin{tikzpicture}[font=\footnotesize,scale=0.95]
    ...
    % Navigator
    \fill  [green!80!black!60] (2,7) rectangle (4.5,5.5);
    \pic[xscale=0.22,yscale=0.3] at (3.25,6.6725) {plane};
    \node at (3.25,5.875) {$\hat{\vec{x}}_{k|k-1}$, $\mat{P}_{k|k-1}$};
    
    % Sensors
    \fill  (1.25,3) [blue!80!black!40] ellipse (0.4 and 0.4);
    \node at (1.25,2.375) {Sensor $1$};
    \fill  (5.25,3) [blue!80!black!40] ellipse (0.4 and 0.4);
    ...
    \node at (3.25,4.5) {$\left\langle\mathcal{E}_{pk_0}(\mathsf{E}_0(\omega)) \,|\, \omega \!\in\! \{\hat{x}^3_{k|k-1}, \hat{y}^3_{k|k-1}, \dots\}\right\rangle$};
    ...
    % Navigator
    \fill  (3.25,0.25) [green!80!black!60] ellipse (0.4 and 0.4);
    \pic[xscale=0.22,yscale=0.3] at (3.25,0.4225) {plane};
        
    % Sensors
    \fill  [blue!80!black!40] (6.25,-3.625)  rectangle (4.25,-2.625);
    \node at (1.25,-3.875) {Sensor $1$};
    \fill  [blue!80!black!40] (2.25,-3.625)  rectangle (0.25,-2.625);
    \node at (5.25,-3.875) {Sensor $n$};
    
    \node at (1.25,-3.125) {$z'_{k,1}$, $r'_{k,1}$};
    \node at (5.25,-3.125) {$z'_{k,n}$, $r'_{k,n}$};
    ...
    \node[align=center] at (1.75,-1.375) {$\mathcal{E}_{pk_0,sk_1}(\mathsf{E}_1(\vec{i}'_{k,1}))$,\\$\mathcal{E}_{pk_0,sk_1}(\mathsf{E}_1(\mat{I}'_{k,1}))$};
    \fill [lightgray]  (3.375,-0.875) rectangle (6.125,-1.875);
    \node[align=center] at (4.75,-1.375) {$\mathcal{E}_{pk_0,sk_n}(\mathsf{E}_1(\vec{i}'_{k,n}))$,\\$\mathcal{E}_{pk_0,sk_n}(\mathsf{E}_1(\mat{I}'_{k,n}))$};
    ...
    % Navigator
    \fill  [green!80!black!60] (1.5,-5.5) rectangle (5,-7.5);
    \pic[xscale=0.22,yscale=0.3] at (3.25,-5.8275) {plane};
    \node at (3.25,-6.625) {$\sum_{i=1}^n \vec{i}'_{k,i}$, $\sum_{i=1}^n\mat{I}'_{k,i}$};
    \node at (3.25,-7.125) {$\hat{\vec{x}}_{k|k}$, $\mat{P}_{k|k}$};
\end{tikzpicture}
...
We have considered multiple sensor layouts, each with four sensors, to capture the dependence of estimated modified measurement variances $r'_{k,i}$ on the original measurements $z_{k,i}$. These layouts of varying sensor distances to the navigator track are shown in figure \ref{fig:sim_layouts}.
\begin{figure}[htbp]
    \centering
...
\begin{definition}
    An encryption scheme meets LCAO security if no probabilistic adversary, running in polynomial-time with respect to security parameter $\kappa$, has more than a negligible advantage in winning the above security game. Probabilities are taken over randomness introduced by $\mathcal{A}$, and in $\mathsf{Setup}$, $\mathsf{Enc}$ and $\mathsf{CombEnc}$.
\end{definition}
