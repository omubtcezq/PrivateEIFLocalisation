...
\begin{equation}
    \mean{y}_k = \mat{P}_k^{-1}\mean{x}_k \quad\textrm{and}\quad \mat{Y}_k = \mat{P}_k^{-1}\,,
\end{equation}
...
\begin{equation}
    \mean{y}_k = \mean{y}_{k-1} + \sum^n_{i=1}\mat{H}^\top_{k,i} \mat{R}^{-1}_i (z_{k,i} - h_i(\mean{x}_k) + \mat{H}_{k,i}\mean{x}_k) \label{eqn:eif_info_vec_update}
\end{equation}
and
\begin{equation}
    \mat{Y}_k = \mat{Y}_{k-1} + \sum^n_{i=1}\mat{H}^\top_{k,i} \mat{R}^{-1}_i \mat{H}_{k,i}\,, \label{eqn:eif_info_mat_update}
\end{equation}
with Jacobian
\begin{equation}
    \mat{H}_{k,i} = \left.\frac{\partial h_i}{\partial \vec{x}}\right|_{\mean{x}_k} \label{eqn:measurement_jacobian}
\end{equation}
for sensors $1 \leq i \leq n$. The filter's prediction step can be computed by converting information vector $\mean{y}_k$ and matrix $\mat{Y}_k$ back to state estimate and covariance 
\begin{equation}
    \mean{x}_k = \mat{Y}_k^{-1}\mean{y}_k \quad\textrm{and}\quad \mat{P}_k = \mat{Y}_k^{-1}\,,
\end{equation}
before using the normal EKF prediction equations
\begin{equation}
    \mean{x}_k = \vec{f}(\mean{x}_{k-1})
\end{equation}
and
\begin{equation}
...
with Jacobian
\begin{equation}
    \mat{F}_{k} = \left.\frac{\partial \vec{f}}{\partial \vec{x}}\right|_{\mean{x}_k}\,.
\end{equation}
...
\begin{equation}
    \vec{i}_i = \mat{H}^\top_{k,i} \mat{R}^{-1}_i (z_{k,i} - h_i(\mean{x}_k) + \mat{H}_{k,i}\mean{x}_k)
\end{equation}
and the measurement matrix
\begin{equation}
    \mat{I}_i = \mat{H}^\top_{k,i} \mat{R}^{-1}_i \mat{H}_{k,i}\,,
\end{equation}
at each time step $k$. All sensitive sensor information is captured exclusively in the filter by the Products $\vec{i}_i$ and $\mat{I}_i$, namely their measurements $z_{k,i}$, measurement variances $\mat{R}_i$ and locations, captured in measurement functions $h_i$ and Jacobians $\mat{H}_{k,i}$. Encryption of $\vec{i}_i$ and $\mat{I}_i$ at the sensors would, however, disclose the location information of $\mean{x}_k$, required for computing Jacobians $\mat{H}_{k,i}$ in \eqref{eqn:measurement_jacobian}.
