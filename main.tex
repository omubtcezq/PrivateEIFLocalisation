...
\end{equation}
where the $H(t)^{sk_i}$ can be considered the noise term which hides the again easily computable discrete logarithm $g^{a^{(t)}_{i}} \pmod{N^2}$, where $g=N+1$.
...
\begin{LaTeXdescription}
    ...
    \item[$\mathsf{CombEnc}(t, pk_0, sk_i, \mathcal{E}_{pk_0}(\omega_1^{(t)}),\dots,\mathcal{E}_{pk_0}(\omega_m^{(t)}), a^{(t)}_{i,1},\dots,a^{(t)}_{i,m})$] At instance $t$, user $i$ computes and obtains the encrypted linear combination $l^{(t)}_i = \mathcal{E}_{pk_0,sk_i}(\sum^m_{j=1}a^{(t)}_{i,j}\omega^{(t)}_j)$ using its secret key $sk_i$.
    \item[$\mathsf{AggDec}(t, pk_0, sk_0, l^{(t)}_1,\dots,l^{(t)}_n)$] At instance $t$, the aggregator computes the aggregation of linear combinations $\sum^{n}_{i=1}l_i^{(t)}=\sum^{n}_{i=1}\sum^{m}_{j=1} a^{(t)}_{i,j}\omega^{(t)}_j$ using its public and private keys $pk_0$, $sk_0$.
\end{LaTeXdescription}
...
\begin{LaTeXdescription}
    ...
    \begin{equation}
        \mathcal{E}_{pk_0}(a) = (N+1)^{x}\rho^N \pmod{N^2}\,, \label{eqn:our_scheme_encrypt}
    \end{equation}
    for a randomly chosen $\rho \in \mathbb{Z}_N$.

    \item[$\mathsf{CombEnc}(t, pk_0, sk_i, \mathcal{E}_{pk_0}(\omega_1^{(t)}),\dots,\mathcal{E}_{pk_0}(\omega_m^{(t)}), a^{(t)}_{i,1},\dots,a^{(t)}_{i,m})$] The linear combination encryption step at instance $t$ is computed as 
    \begin{equation}
    ...
    \item[$\mathsf{AggDec}(t, pk_0, sk_0, l^{(t)}_1,\dots,l^{(t)}_n)$] Aggregation is computed as $l^{(t)} = \prod^n_{i=1}l^{(t)}_i \pmod{N^2}$, removing aggregation noise terms, and is followed by Paillier decryption
    \begin{equation}
        \begin{split}
...
\begin{equation}
    \mathbb{Q}_{M,\phi} = \left\{q \,\middle|\, \phi q \in \mathbb{N} \wedge -\left\lfloor\frac{M}{2}\right\rfloor \leq q < \left\lfloor\frac{M}{2}\right\rfloor \right\}\,,
\end{equation}
...
The function for combined quantisation and encoding $\mathsf{E}_{M,\phi,d}(a)$, of a given a real number $a \in \mathbb{R}$, integer range $\mathbb{Z}_M$, and the desired scaling for $d$ prior encoded multiplications, is given by
\begin{equation}
...
The choice of a high fractional precision $\phi$ may reduce quantisation errors introduced in \eqref{eqn:encode}, however, risks an overflow following too many multiplications. Given the largest number of expected multiplications $d_{max}$, and the largest expected decoded value $a$, the parameter should be chosen such that the following condition holds
\begin{equation}
    \left|\phi^{d_{max}+1}a\right| < \left\lfloor \frac{M}{2} \right\rfloor\,.
\end{equation}
...
\begin{algorithmic}[1]
    ...
    \For{$v \gets 1$ to $2$}
        \State Send $\alpha_{v}$ to the navigator
    \EndFor
...
\begin{LaTeXdescription}
    ...
    \item[$\mathsf{Enc}(t, sk_i, a^{(t)}_i)$] At instance $t$, user $i$ computes and obtains the encrypted value $l^{(t)}_i = \mathcal{E}_{sk_i}(a^{(t)}_i)$ using its secret key $sk_i$.
    \item[$\mathsf{AggDec}(t, sk_0, l^{(t)}_1,\dots,l^{(t)}_n)$] At instance $t$, the aggregator computes the aggregation of values $\sum^{n}_{i=1} a^{(t)}_i$ using its private key $sk_0$.
\end{LaTeXdescription}
