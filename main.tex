...
\begin{align*}
    ...
    \end{split}\\
    \alpha_i^{(k,5)} &= \beta_{12}\ \textrm{and} \\
...
\begin{LaTeXdescription}
    \item[Setup] The $\mathsf{Setup}$ algorithm from section \ref{sec:lcao_scheme} is run only once by a trusted third party, $N$ and $H$ are made public, and the navigator and sensor secret keys, $sk_0=\lambda=\mathsf{lcm}(p-1, q-1)$ and $sk_i$, $i\in\{1,\dots,n\}$, are distributed accordingly. 
