...
\end{LaTeXdescription}
The security notions we want the above algorithms to meet reflect the previously stated estimation privacy goals. The navigator should learn no information from individual sensors while sensors should learn no information from the navigator or any other sensors. In the context of the introduced communication protocol, this can be summarised as the following notions.
\begin{LaTeXdescription}
...
\end{equation}
with noise term $\vec{w}_k$. Measurements of the system follow a measurement model dependent on sensor $i\in\{1,\dots,n\}$, given by 
\begin{equation}
...
The function for \textit{combined} quantization and encoding, $\mathsf{E}_{M,\phi,d}(a)$, of a given number $a \in \mathbb{R}$ and with an integer range $\mathbb{Z}_M$, precision $\phi$ and scaling for $d$ prior encoded multiplications is given by
\begin{equation}
...
A difference, however, is that we intend to use our scheme for the aggregation of multidimensional data from a localisation estimator. As the scheme is only defined for the aggregation of scalar inputs, we compute 

the concatenation $t_{k,\tau}=k\mathbin\|\tau$ 
...
\begin{LaTeXdescription}
    ...
    \begin{equation*}
        ...
        \end{split}
    \end{equation*}
    for some values $\rho_i,\rho_j \in \mathbb{Z}_N,\,j\in\{1,\dots,m\}$. Here, $\rho_i^N$ and $H(t)^{sk_i}$ can be considered the noise terms corresponding to the two levels of encryption from $pk_0$ and $sk_i$, respectively.
...
\begin{remark}
    Given the construction of the scheme above, it can be seen that any weights $\omega^{(t)}_j$, whose values are known at each sensor, do not need to be broadcast by the navigator. In this case, sensors can replace
    \begin{equation}
        \mathcal{E}_{pk_0}(\omega^{(t)}_j)^{a^{(t)}_{i,j}} = (N+1)^{\omega^{(t)}_ja^{(t)}_{i,j}}\rho_j^N \pmod{N^2}
    \end{equation}
    in \eqref{eqn:our_scheme_lin_comb}, by
    \begin{equation}
        (N+1)^{\omega^{(t)}_ja^{(t)}_{i,j}} \pmod{N^2}\,.
    \end{equation}
    This is due to the removal of $\rho_j^N$ terms during decryption and can be used to reduce the navigator's broadcast communication cost by the number of weights $\omega^{(t)}_j$ that do not hold any information private to the navigator and are known by the sensors in advance.
\end{remark}
...
\begin{equation}
    \begin{split}
        ...
        &=
        \begin{bmatrix}
            \beta_1^{(i)} & \beta_2^{(i)} & 0 & \cdots \\
            \beta_3^{(i)} & \beta_4^{(i)} & 0 & \cdots\\
            0 & 0 & 0 & \cdots \\
...
\begin{LaTeXdescription}
    \item[Setup] Run only once, the $\mathsf{Setup}$ algorithm from section \ref{sec:lcao_scheme} is run by a trusted third party, $N$ and $H$ are made public, and the navigator and sensor secret keys, $sk_0=\lambda=\mathsf{lcm}(p-1, q-1)$ and $sk_i$, $i\in\{1,\dots,n\}$, are distributed accordingly. 
...
\begin{algorithmic}[1]
    ...
    \For{$v \gets 1$ to $2$}
        \State Receive $\bm{\alpha}_{v}^{(i)}$ from each sensor $i\in\{1,\dots,n\}$
    \EndFor

    \For{$w \gets 1$ to $4$}
        \State Receive $\bm{\beta}_{w}^{(i)}$ from each sensor $i\in\{1,\dots,n\}$
    \EndFor
...
\end{equation*}
for \textit{all} negligible functions $\eta_1$, $\eta_2$ and security parameter $\kappa$. That is, if we assume our scheme is not LCAO secure, then the Joye-Libert scheme is not AO secure (which is not the case, \cite{joyeScalableSchemePrivacyPreserving2013}).
\begin{proof}
